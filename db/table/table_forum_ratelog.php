...
class table_forum_ratelog extends discuz_table
{
	...
	public function fetch_by_uid_pid($uid, $pid) {
		return Dxyz_DB::fetch_first('SELECT * FROM %t WHERE uid=%d AND pid=%d LIMIT 1', array($this->_table, $uid, $pid));
	}
	public function fetch_all_by_pid($pid, $sort = 'DESC') {
		...
		return Dxyz_DB::fetch_all("SELECT * FROM %t WHERE $wheresql ORDER BY dateline $sort", array($this->_table, $pid));
	}

	public function fetch_all_sum_score($uid, $dateline) {
		return Dxyz_DB::fetch_all('SELECT extcredits, SUM(ABS(score)) AS todayrate FROM %t WHERE uid=%d AND dateline>=%d GROUP BY extcredits', array($this->_table, $uid, $dateline));
	}

	public function count_by_uid_pid($uid, $pid) {
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE uid=%d AND pid=%d LIMIT 1', array($this->_table, $uid, $pid));
	}
	...
	public function delete_by_pid_uid_extcredits_dateline($pid = null, $uid = null, $extcredits = null, $dateline = null) {
		...
		if(!empty($wherearr)) {
			...
			return Dxyz_DB::query("DELETE FROM %t $wheresql", $parameter, true, true);
		}
		return false;
	}
	...
	public function fetch_postrate_by_pid($pids, $postlist, $postcache, $ratelogrecord) {
		...
		$query = Dxyz_DB::query("SELECT * FROM ".Dxyz_DB::table('forum_ratelog')." WHERE pid IN (".dimplode($pids).") ORDER BY dateline DESC");
		$ratelogs = array();
		while($ratelog = Dxyz_DB::fetch($query)) {
