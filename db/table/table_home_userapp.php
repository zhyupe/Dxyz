...
class table_home_userapp extends discuz_table
{
	...
	public function fetch_by_uid_appid($uid, $appid) {
		return Dxyz_DB::fetch_first('SELECT * FROM %t WHERE uid=%d AND appid=%d', array($this->_table, $uid, $appid));
	}

	public function fetch_max_menuorder_by_uid($uid) {
		return Dxyz_DB::result_first('SELECT MAX(menuorder) FROM %t WHERE uid=%d', array($this->_table, $uid));
	}
	...
	public function fetch_all_by_uid_appid($uid = 0, $appid = 0, $order = null, $sort = 'DESC' , $start = 0, $limit = 0) {
		...
		$ordersql = $order !== null ? ' ORDER BY '.Dxyz_DB::order($order, $sort) : '';
		if(!$uid) {
			$limit = $limit ? $limit : 100;
		}
		return Dxyz_DB::fetch_all("SELECT * FROM %t $wheresql $ordersql ".Dxyz_DB::limit($start, $limit), $parameter);
	}
	...
	public function update_by_uid_appid($uid = 0, $appid = 0, $data = array()) {
		if($data && is_array($data)) {
			...
			if($appid) {
				$appid = dintval((array)$appid, true);
				$wherearr[] = Dxyz_DB::field('appid', $appid);
			}
			$wheresql = !empty($wherearr) && is_array($wherearr) ? implode(' AND ', $wherearr) : '';
			return Dxyz_DB::update($this->_table, $data, $wheresql);
		}
		return 0;
	}
	...
	public function delete_by_uid_appid($uid = 0, $appid = 0) {
		...
		$wheresql = !empty($wherearr) && is_array($wherearr) ? ' WHERE '.implode(' AND ', $wherearr) : '';
		return Dxyz_DB::query("DELETE FROM %t $wheresql", $parameter);
	}
