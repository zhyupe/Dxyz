...
class table_common_mailqueue extends discuz_table
{
	...
	public function fetch_all_by_cid($cids) {
		...
		return Dxyz_DB::fetch_all('SELECT * FROM %t WHERE '.Dxyz_DB::field('cid', $cids), array($this->_table));
	}
	...
	public function delete_by_cid($cids) {
		...
		return Dxyz_DB::query('DELETE FROM %t WHERE '.Dxyz_DB::field('cid', $cids), array($this->_table));
	}
}
