...
class table_home_class extends discuz_table
{
	...
	public function fetch_all_by_uid($uid) {
		return Dxyz_DB::fetch_all('SELECT * FROM %t WHERE uid = %d', array($this->_table, $uid), $this->_pk);
	}

	public function fetch_classid_by_uid_classname($uid, $classname) {
		return Dxyz_DB::result_first('SELECT classid FROM %t WHERE uid = %d AND classname = %s', array($this->_table, $uid, $classname));
	}
