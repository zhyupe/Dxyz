...
class table_common_member_archive extends table_common_member
{
	...
	public function fetch($id){
		$data = array();
		if(isset($this->membersplit) && ($id = dintval($id)) && ($data = Dxyz_DB::fetch_first('SELECT * FROM '.Dxyz_DB::table($this->_table).' WHERE '.Dxyz_DB::field($this->_pk, $id)))) {
	...
	public function fetch_by_username($username) {
		$user = array();
		if(isset($this->membersplit) && $username && ($user = Dxyz_DB::fetch_first('SELECT * FROM %t WHERE username=%s', array($this->_table, $username)))) {
	...
	public function count() {
		return isset($this->membersplit) ? Dxyz_DB::result_first('SELECT COUNT(*) FROM %t', array($this->_table)) : 0;
	}

	public function fetch_by_email($email) {
		$user = array();
		if(isset($this->membersplit) && $email && ($user = Dxyz_DB::fetch_first('SELECT * FROM %t WHERE email=%s', array($this->_table, $email)))) {
			$user['_inarchive'] = true;
		}
		return $user;
	}
	...
	public function move_to_master($uid){
		if(isset($this->membersplit) && ($uid = intval($uid)) && ($member = $this->fetch($uid))) {
			...
			Dxyz_DB::insert('common_member',$member);
	...
	public function delete($val, $unbuffered = false) {
		return isset($this->membersplit) && ($val = dintval($val, true)) && Dxyz_DB::delete($this->_table, Dxyz_DB::field($this->_pk, $val), null, $unbuffered);
	}

	public function check_table() {
		if(Dxyz_DB::fetch_first("SHOW TABLES LIKE '".Dxyz_DB::table('common_member_archive')."'")){
			return false;
		} else {
			...
			foreach($mastertables as $tablename) {
				$createtable = Dxyz_DB::fetch_first('SHOW CREATE TABLE '.Dxyz_DB::table($tablename));
				Dxyz_DB::query(str_replace(Dxyz_DB::table($tablename), Dxyz_DB::table("{$tablename}_archive"), $createtable['Create Table']));
			}
			return true;
		}
	}
	...
	public function rebuild_table($step) {
		...
		$updates = array();
		$mastertable = Dxyz_DB::table($mastertables[$step]);
		$archivetable = Dxyz_DB::table($mastertables[$step].'_archive');

		$mastercols = Dxyz_DB::fetch_all('SHOW COLUMNS FROM '.$mastertable, null, 'Field');
		$archivecols = Dxyz_DB::fetch_all('SHOW COLUMNS FROM '.$archivetable, null, 'Field');
		foreach(array_diff(array_keys($archivecols), array_keys($mastercols)) as $field) {
			$updates[] = "DROP `$field`";
		}

		$createtable = Dxyz_DB::fetch_first('SHOW CREATE TABLE '.$mastertable);
		$mastercols = $this->getcolumn($createtable['Create Table']);

		$archivecreatetable = Dxyz_DB::fetch_first('SHOW CREATE TABLE '.$archivetable);
		...
		$ret = true;
		if(!empty($updates)) {
			if(!Dxyz_DB::query("ALTER TABLE ".$archivetable." ".implode(', ', $updates), 'SILENT')) {
				$ret = $mastertable;
			} else {
			}
		}
