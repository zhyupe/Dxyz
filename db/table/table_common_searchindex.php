...
class table_common_searchindex extends discuz_table
{
	...
	public function fetch_by_searchid_srchmod($searchid, $srchmod) {
		return Dxyz_DB::fetch_first('SELECT * FROM %t WHERE searchid=%d AND srchmod=%d', array($this->_table, $searchid, $srchmod));
	}

	public function count_by_dateline($timestamp, $srchmod = '') {
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE '.($srchmod ? 'srchmod='.dintval($srchmod).' AND ' : '').'dateline>%d-60', array($this->_table, $timestamp));
	}
	...
	public function fetch_all_search($searchctrl, $useip, $uid, $timestamp, $searchstring, $srchmod = '') {
		...
		return Dxyz_DB::fetch_all("SELECT searchid, dateline,
			...
			FROM ".Dxyz_DB::table($this->_table)."
