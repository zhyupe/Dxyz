...
class table_forum_memberrecommend extends discuz_table
{
	...
	public function fetch_by_recommenduid_tid($uid, $tid) {
		return Dxyz_DB::fetch_first('SELECT * FROM %t WHERE recommenduid=%d AND tid=%d', array($this->_table, $uid, $tid));
	}

	public function count_by_recommenduid_dateline($uid, $dateline) {
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE recommenduid=%d AND dateline>%d',array($this->_table, $uid, $dateline));
	}
