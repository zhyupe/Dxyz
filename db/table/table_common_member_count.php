...
class table_common_member_count extends discuz_table_archive
{
	...
	public function count_by_posts($num) {
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE posts=%d', array($this->_table, $num));
	}
	...
	public function range_by_field($start = 0, $limit = 0, $orderby = '', $sort = '') {
		...
		return Dxyz_DB::fetch_all('SELECT * FROM '.Dxyz_DB::table($this->_table).($orderby ? ' ORDER BY '.Dxyz_DB::order($orderby, $sort) : '').Dxyz_DB::limit($start, $limit), null, $this->_pk);
	}

	public function clear_digestposts() {
		$uids = array();
		if($this->_allowmem) {
			$uids = Dxyz_DB::fetch_all('SELECT uid FROM '.Dxyz_DB::table($this->_table).' WHERE digestposts<>0', null, $this->_pk);
		}
		$data = Dxyz_DB::query("UPDATE ".Dxyz_DB::table($this->_table)." SET digestposts=0", 'UNBUFFERED');
		if(!empty($uids)) {
	...
	public function clear_today_data() {
		$uids = array();
		if($this->_allowmem) {
			$uids = Dxyz_DB::fetch_all('SELECT uid FROM '.Dxyz_DB::table($this->_table).' WHERE todayattachs<>0 OR todayattachsize<>0', null, $this->_pk);
		}
		$data = Dxyz_DB::query("UPDATE ".Dxyz_DB::table($this->_table)." SET todayattachs='0',todayattachsize='0'", 'UNBUFFERED');
		if(!empty($uids)) {
	...
	public function count_by_extcredits($extcredits, $credits) {
		...
		if(in_array($extcredits, array(1,2,3,4,5,6,7,8))) {
			$count =  Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE extcredits'.$extcredits.'>%d', array($this->_table, $credits));
		}
		return $count;
	}
	...
	public function count_by_friends($friends) {
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE friends>%d', array($this->_table, $friends));
	}
}
