...
class table_portal_article_count extends discuz_table
{
	...
	public function increase($ids, $data) {
		...
		if(!empty($sql)){
			Dxyz_DB::query("UPDATE ".Dxyz_DB::table($this->_table)." SET ".implode(',', $sql)." WHERE aid IN (".dimplode($ids).")", 'UNBUFFERED');
		}
	}
	...
	public function fetch_all_hotarticle($wheresql, $dateline) {
		if(!empty($wheresql) && ($wheresql = (string)$wheresql) && $dateline = dintval($dateline)) {
			return Dxyz_DB::fetch_all("SELECT at.* FROM ".Dxyz_DB::table($this->_table)." ac, ".Dxyz_DB::table('portal_article_title')." at WHERE $wheresql AND at.dateline>'$dateline' AND ac.aid=at.aid ORDER BY ac.viewnum DESC LIMIT 10");
		} else {
			return array();
		}
	}
