...
class table_forum_postcomment extends discuz_table
{
	...
	public function count_by_tid($tid, $authorid = null, $score = null) {
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE tid=%d '.($authorid ? ' AND '.Dxyz_DB::field('authorid', $authorid) : null).($score ? ' AND '.Dxyz_DB::field('score', $score) : null), array($this->_table, $tid, $authorid, $score));
	}
