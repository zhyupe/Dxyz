...
class table_common_taskvar extends discuz_table
{
	...
	public function fetch_all_by_taskid($taskid, $variable = '') {
		...
		$variable = $variable ? ' AND '.($variable !== 'IS NOT NULL' ? Dxyz_DB::field('variable', $variable) : 'variable IS NOT NULL') : '';
		return Dxyz_DB::fetch_all("SELECT * FROM %t WHERE %i%i", array($this->_table, Dxyz_DB::field('taskid', $taskid), $variable));
	}
	...
	public function update_by_taskid($taskid, $variable, $val) {
		...
		return Dxyz_DB::update($this->_table, $val, array('taskid' => $taskid, 'variable' => $variable), 'UNBUFFERED');
	}
	...
	public function delete_by_taskid($taskid) {
		...
		return Dxyz_DB::delete($this->_table, Dxyz_DB::field('taskid', $taskid));
	}
