...
class table_common_credit_log extends discuz_table
{
	...
	public function fetch_by_operation_relatedid($operation, $relatedid) {
		...
		return Dxyz_DB::fetch_all('SELECT * FROM %t WHERE '.implode(' AND ', $wherearr), $parameter);
	}
	public function fetch_all_by_operation($operation, $start = 0, $limit = 0) {
		return Dxyz_DB::fetch_all('SELECT * FROM %t WHERE operation=%s ORDER BY dateline DESC '.Dxyz_DB::limit($start, $limit), array($this->_table, $operation));
	}
	public function fetch_all_by_uid_operation_relatedid($uid, $operation, $relatedid) {
		...
		$parameter[] = $operation;
		$parameter[] = $relatedid;
		return Dxyz_DB::fetch_all('SELECT * FROM %t WHERE '.implode(' AND ', $wherearr).' ORDER BY dateline', $parameter);
	}
	public function fetch_all_by_uid($uid, $start = 0, $limit = 0) {
		$array = Dxyz_DB::fetch_all('SELECT * FROM %t WHERE uid=%d ORDER BY dateline DESC '.Dxyz_DB::limit($start, $limit), array($this->_table, $uid), 'logid');
		if(!$array) {
			return array();
		}
		$fieldids = array();
		...
		if($fieldids) {
			$arrayfield = Dxyz_DB::fetch_all('SELECT * FROM %t WHERE logid IN(%n)', array('common_credit_log_field', $fieldids), 'logid');
	...
	public function fetch_all_by_search($uid, $optype, $begintime = 0, $endtime = 0, $exttype = 0, $income = 0, $extcredits = array(), $start = 0, $limit = 0, $relatedid = 0) {
		...
		$array = Dxyz_DB::fetch_all('SELECT * FROM %t '.$condition[0].' ORDER BY dateline DESC '.Dxyz_DB::limit($start, $limit), $condition[1], 'logid');
		if(!$array) {
			return array();
		}
		$fieldids = array();
	...
	public function delete_by_operation_relatedid($operation, $relatedid) {
		...
		if($operation && $relatedid) {
			return Dxyz_DB::delete($this->_table, Dxyz_DB::field('operation', $operation).' AND '.Dxyz_DB::field('relatedid', $relatedid));
		}
		return 0;
	}
	...
	public function delete_by_uid_operation_relatedid($uid, $operation, $relatedid) {
		...
		if($relatedid && $uid && $operation) {
			return Dxyz_DB::delete($this->_table, Dxyz_DB::field('uid', $uid).' AND '.Dxyz_DB::field('operation', $operation).' AND '.Dxyz_DB::field('relatedid', $relatedid));
		}
		return 0;
	}
	public function update_by_uid_operation_relatedid($uid, $operation, $relatedid, $data) {
		...
		if(!empty($data) && is_array($data) && $relatedid && $uid && $operation) {
			return Dxyz_DB::update($this->_table, $data, Dxyz_DB::field('uid', $uid).' AND '.Dxyz_DB::field('operation', $operation).' AND '.Dxyz_DB::field('relatedid', $relatedid));
		}
		return 0;
	}
	public function count_by_uid_operation_relatedid($uid, $operation, $relatedid) {
		...
		if($relatedid && $uid && $operation) {
			...
			$wherearr[] = is_array($operation) && $operation ? 'operation IN(%n)' : 'operation=%s';
			$wherearr[] = is_array($relatedid) && $relatedid ? 'relatedid IN(%n)' : 'relatedid=%d';
			return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE '.implode(' AND ', $wherearr), array($this->_table, $uid, $operation, $relatedid));
		}
		return 0;
	}
	...
	public function count_by_operation($operation) {
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE operation=%s', array($this->_table, $operation));
	}
	public function count_stc_by_relatedid($relatedid, $creditid, $operation = 'STC') {
		$creditid = intval($creditid);
		if($creditid) {
			return Dxyz_DB::fetch_first("SELECT COUNT(*) AS payers, SUM(extcredits%d) AS income FROM %t WHERE relatedid=%d AND operation=%s", array($creditid, $this->_table, $relatedid, $operation));
		}
		return 0;
	}
	public function count_credit_by_uid_operation_relatedid($uid, $operation, $relatedid, $creditid) {
		...
		if($creditid) {
			...
			$wherearr[] = is_array($operation) && $operation ? 'operation IN(%n)' : 'operation=%s';
			$wherearr[] = is_array($relatedid) && $relatedid ? 'relatedid IN(%n)' : 'relatedid=%d';
			return Dxyz_DB::result_first('SELECT SUM(extcredits%d) AS credit FROM %t WHERE '.implode(' AND ', $wherearr), array($creditid, $this->_table, $uid, $operation, $relatedid));
		}
		return 0;
	}
