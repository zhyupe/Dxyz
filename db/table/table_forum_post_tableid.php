...
class table_forum_post_tableid extends discuz_table
{
	...
	public function alter_auto_increment($auto_increment) {
		return Dxyz_DB::query("ALTER TABLE %t AUTO_INCREMENT=%d", array($this->_table, $auto_increment));
	}

	public function delete_by_lesspid($pid) {
		return Dxyz_DB::query("DELETE FROM %t WHERE pid<%d", array($this->_table, $pid));
	}

	public function fetch_max_id() {
		return Dxyz_DB::result_first('SELECT MAX(pid) FROM '.Dxyz_DB::table($this->_table));
	}
}
