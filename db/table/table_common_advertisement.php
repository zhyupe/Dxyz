...
class table_common_advertisement extends discuz_table
{
	...
	public function fetch_all_type() {
		return Dxyz_DB::fetch_all("SELECT type, COUNT(type) AS count FROM %t GROUP BY type", array($this->_table));
	}

	public function fetch_all_by_type($type) {
		return Dxyz_DB::fetch_all("SELECT * FROM %t WHERE type=%s", array($this->_table, $type));
	}

	public function fetch_all_old() {
		return Dxyz_DB::fetch_all("SELECT * FROM %t WHERE available>0 AND starttime<=%d ORDER BY displayorder", array($this->_table, TIMESTAMP));
	}

	public function close_endtime() {
		Dxyz_DB::update($this->_table, array('available' => 0), "endtime>0 AND endtime<='".TIMESTAMP."'", 'UNBUFFERED');
	}

	public function fetch_all_endtime($endtime) {
		return Dxyz_DB::fetch_all("SELECT * FROM %t WHERE endtime=%s", array($this->_table, $endtime));
	}

	private function _search_conditions($title, $starttime, $endtime, $type, $target) {
		$conditions = '';
		$conditions .= $title ? " AND ".Dxyz_DB::field('title', '%'.$title.'%', 'like') : '';
		...
		$conditions .= $type ? " AND ".Dxyz_DB::field('type', $type) : '';
		$conditions .= $target ? " AND ".Dxyz_DB::field('targets', '%'.$target.'%', 'like') : '';
		return $conditions;
	}
	...
	public function fetch_all_search($title, $starttime, $endtime, $type, $target, $orderby, $start_limit, $advppp) {
		...
		return Dxyz_DB::fetch_all("SELECT * FROM ".Dxyz_DB::table('common_advertisement')." WHERE 1 $conditions ORDER BY available DESC, $order_by LIMIT $start_limit, $advppp");
	}

	public function count_search($title, $starttime, $endtime, $type, $target) {
		$conditions = $this->_search_conditions($title, $starttime, $endtime, $type, $target);
		return Dxyz_DB::result_first("SELECT COUNT(*) FROM ".Dxyz_DB::table('common_advertisement')." WHERE 1 $conditions");
	}
