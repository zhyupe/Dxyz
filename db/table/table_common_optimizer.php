...
class table_common_optimizer extends discuz_table {
	...
	public function fetch($skey, $auto_unserialize = false) {
		$data = Dxyz_DB::result_first('SELECT v FROM '.Dxyz_DB::table($this->_table).' WHERE '.Dxyz_DB::field($this->_pk, $skey));
		return $auto_unserialize ? (array)unserialize($data) : $data;
	}

	public function update($skey, $svalue){
		return Dxyz_DB::insert($this->_table, array($this->_pk => $skey, 'v' => is_array($svalue) ? serialize($svalue) : $svalue), false, true);
	}

	public function fetch_all($skeys) {
		return Dxyz_DB::fetch_all('SELECT * FROM '.Dxyz_DB::table($this->_table).' WHERE '.Dxyz_DB::field($this->_pk, $skeys));
	}
}
?>