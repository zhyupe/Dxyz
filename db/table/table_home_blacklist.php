...
class table_home_blacklist extends discuz_table
{
	...
	public function fetch_all_by_uid_buid($uid, $buids) {
		return Dxyz_DB::fetch_all('SELECT * FROM %t WHERE uid=%d AND buid IN(%n)', array($this->_table, $uid, $buids), 'buid');
	}

	public function delete_by_uid_buid($uid, $buid) {
		return Dxyz_DB::query('DELETE FROM %t WHERE uid=%d AND buid=%d', array($this->_table, $uid, $buid));
	}
}
