...
class table_portal_comment extends discuz_table
{
	...
	public function fetch_all_by_id_idtype($id, $idtype = '', $orderby = '', $ordersc = 'DESC', $start = 0, $limit = 0) {
		...
		$sql = array(Dxyz_DB::field('id', $id));
		if($idtype) {
			$sql[] = Dxyz_DB::field('idtype', $idtype);
		}
		...
		return Dxyz_DB::fetch_all('SELECT * FROM %t WHERE %i', array($this->_table, $wheresql));
	}
	...
	public function count_by_id_idtype($id, $idtype) {
		...
		$sql = Dxyz_DB::field('id', $id).' AND '.Dxyz_DB::field('idtype', $idtype);
	...
	public function delete_by_id_idtype($id, $idtype) {
		...
		$para = Dxyz_DB::field('id', $id);
		if($idtype) {
			$para .= ' AND '.Dxyz_DB::field('idtype', $idtype);
		}
		return Dxyz_DB::delete($this->_table, $para);
	}
	...
	public function fetch_all_by_search($aid, $authorid, $starttime, $endtime, $idtype, $message, $start = 0, $limit = 0, $type = 1) {
		...
		$sql = '';
		$sql .= $aid ? ' AND c.'.Dxyz_DB::field('id', $aid) : '';
		$sql .= $authorid ? ' AND c.'.Dxyz_DB::field('uid', $authorid) : '';
		$sql .= $starttime ? ' AND c.'.Dxyz_DB::field('dateline', $starttime, '>') : '';
		$sql .= $endtime ? ' AND c.'.Dxyz_DB::field('dateline', $endtime, '<') : '';
		...
		if($type == 2) {
			return Dxyz_DB::result_first('SELECT count(*) FROM %t c WHERE 1 %i', array($this->_table, $sql));
		} else {
			return Dxyz_DB::fetch_all('SELECT c.*, a.title FROM %t c LEFT JOIN %t a ON a.`'.$idtype.'`=c.id WHERE 1 %i ORDER BY c.dateline DESC %i', array($this->_table, $tablename, $sql, Dxyz_DB::limit($start, $limit)));
		}
	}
