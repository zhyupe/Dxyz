...
class table_common_member_verify_info extends discuz_table
{
	...
	public function fetch_by_uid_verifytype($uid, $verifytype) {
		return Dxyz_DB::fetch_first('SELECT * FROM %t WHERE uid=%d AND verifytype=%d', array($this->_table, $uid, $verifytype));
	}
	public function fetch_all_search($uid, $vid, $flag = null, $username = '', $starttime = 0, $endtime = 0, $order = 'dateline', $start = 0, $limit = 0, $sort = 'DESC') {
		...
		return Dxyz_DB::fetch_all("SELECT * FROM %t $condition[0] $ordersql ".Dxyz_DB::limit($start, $limit), $condition[1], $this->_pk);
	}
	public function group_by_verifytype_count() {
		return Dxyz_DB::fetch_all('SELECT verifytype, COUNT(*) AS num FROM %t WHERE flag=0 GROUP BY verifytype', array($this->_table));
	}
