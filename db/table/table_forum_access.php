...
class table_forum_access extends discuz_table
{
	...
	public function fetch_all_by_fid_uid($fid = 0, $uid = 0, $count = 0, $start = 0, $limit = 0) {
		$uid = intval($uid);
		$sql = $uid ? ' uid='.$uid : '';
		$sql .= $fid ? ($sql ? ' AND ' : '').Dxyz_DB::field('fid', $fid) : '';
		if(empty($sql)) {
			return false;
		}
		if($count) {
	...
	public function fetch_all_by_uid($uid) {
		$data = array();
		if($uid) {
			$data = Dxyz_DB::fetch_all('SELECT * FROM %t WHERE uid=%d', array($this->_table, $uid), 'fid');
		}
		return $data;
	}

	public function count_by_uid($uid) {
		return $uid ? Dxyz_DB::result_first('SELECT count(*) FROM %t WHERE uid=%d', array($this->_table, $uid)) : 0;
	}

	public function delete_by_fid($fid, $uid = 0) {
		$uid = intval($uid);
		$uidsql = $uid ? ' uid='.$uid.' AND ' : '';
		Dxyz_DB::query("DELETE FROM %t WHERE $uidsql fid=%d", array($this->_table, $fid));
	}
	...
	public function update_for_uid($uid, $fid, $data) {
		...
		Dxyz_DB::update($this->_table, $data, Dxyz_DB::field('uid', $uid).' AND '.Dxyz_DB::field('fid', $fid));
	}
