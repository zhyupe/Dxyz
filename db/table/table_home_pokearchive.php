...
class table_home_pokearchive extends discuz_table
{
	...
	public function fetch_all_by_pokeuid($pokeuid) {
		return Dxyz_DB::fetch_all('SELECT * FROM %t WHERE pokeuid=%d ORDER BY dateline', array($this->_table, $pokeuid));
	}
	public function delete_by_dateline($dateline) {
		Dxyz_DB::query('DELETE FROM %t WHERE dateline<%d', array($this->_table, $dateline));
	}
