...
class table_forum_polloption extends discuz_table
{
	...
	public function update_vote($polloptionids, $voterids, $num = 1) {
		Dxyz_DB::query('UPDATE %t SET votes=votes+\'%d\', voterids=CONCAT(voterids,%s) WHERE polloptionid IN (%n)', array($this->_table, $num, $voterids, $polloptionids), false, true);
	}
	public function fetch_all_by_tid($tids, $displayorder = 0, $limit = 0) {
		...
		return Dxyz_DB::fetch_all('SELECT * FROM %t WHERE '.Dxyz_DB::field('tid', $tids).$sqladd, array($this->_table));
	}
