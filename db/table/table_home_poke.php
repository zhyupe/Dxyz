...
class table_home_poke extends discuz_table
{
	...
	public function fetch_all_by_uid_fromuid($uid, $fromuid) {
		...
		return Dxyz_DB::fetch_all('SELECT * FROM %t '.$wheresql, array($this->_table, $uid, $fromuid));
	}
	...
	public function delete_by_uid_or_fromuid($uids) {
		...
		if($uids) {
			return Dxyz_DB::delete($this->_table, Dxyz_DB::field('uid', $uids).' OR '.Dxyz_DB::field('fromuid', $uids));
		}
		return 0;
	}
