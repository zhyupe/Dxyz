...
class table_common_word extends discuz_table
{
	...
	public function fetch_by_find($find) {
		return Dxyz_DB::fetch_first("SELECT * FROM %t WHERE find=%s", array($this->_table, $find));
	}
	public function fetch_all_order_type_find() {
		return Dxyz_DB::fetch_all('SELECT * FROM %t ORDER BY type ASC, find ASC', array($this->_table), $this->_pk);
	}

	public function fetch_all() {
		return Dxyz_DB::fetch_all('SELECT * FROM %t', array($this->_table), $this->_pk);
	}
	...
	public function fetch_all_by_type_find($type = null, $find = null , $start = 0, $limit = 0) {
		...
		return Dxyz_DB::fetch_all("SELECT * FROM %t $wheresql ORDER BY find ASC".Dxyz_DB::limit($start, $limit), $parameter);
	}
	...
	public function update_by_type($types, $data) {
		if(!empty($types) && !empty($data) && is_array($data)) {
			...
			return Dxyz_DB::update($this->_table, $data, "type IN (".dimplode($types).")");
		}
		return 0;
	}
	public function update_by_find($find, $data) {
		if(!empty($find) && !empty($data) && is_array($data)) {
			return Dxyz_DB::update($this->_table, $data, Dxyz_DB::field('find', $find));
		}
		return 0;
	}
