...
class table_common_block_item_data extends discuz_table
{
	...
	public function count_by_bid($bid, $isverified = 1) {
		return ($bid = dintval($bid, true)) ? Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE '.Dxyz_DB::field('bid', $bid).' AND isverified=%d', array($this->_table, $isverified)) : 0;
	}
	...
	public function delete_by_dataid_bid($dataids, $bid) {
		if(($dataids = dintval($dataids, true)) && ($dataids = Dxyz_DB::fetch_all('SELECT dataid FROM %t WHERE dataid IN (%n) AND bid=%d', array($this->_table, $dataids, $bid), $this->_pk))) {
			$this->delete(array_keys($dataids));
		}
	}

	public function fetch_by_bid_id_idtype($bid, $id, $idtype) {
		return $bid && $id && $idtype ? Dxyz_DB::fetch_first('SELECT * FROM %t WHERE bid=%d AND id=%d AND idtype=%s', array($this->_table, $bid, $id, $idtype)) : array();
	}
}
