...
class table_home_doing extends discuz_table
{
	...
	public function update_replynum_by_doid($inc_replynum, $doid) {
		return Dxyz_DB::query('UPDATE %t SET replynum=replynum+\'%d\' WHERE doid=%d', array($this->_table, $inc_replynum, $doid));
	}
	...
	public function fetch_all_by_uid_doid($uids, $bannedids = '', $paramorderby = '', $startrow = 0, $items = 0, $status = true, $allfileds = false) {
		...
		$orderby = $paramorderby && in_array($paramorderby, array('dateline', 'replynum')) ? 'ORDER BY '.Dxyz_DB::order($paramorderby, 'DESC') : 'ORDER BY '.Dxyz_DB::order('dateline', 'DESC');
		...
		return Dxyz_DB::fetch_all('SELECT '.($allfileds ? '*' : 'doid').' FROM %t '.$wheresql.' '.$orderby.Dxyz_DB::limit($startrow, $items), $parameter);
	}
