...
class table_forum_groupcreditslog extends discuz_table
{
	...
	public function check_logdate($fid, $uid, $logdate) {
		return Dxyz_DB::result_first("SELECT logdate FROM %t WHERE fid=%d AND uid=%d AND logdate=%s", array($this->_table, $fid, $uid, $logdate));
	}
	public function delete_by_fid($fid) {
		if(empty($fid)) {
			return false;
		}
		Dxyz_DB::query("DELETE FROM ".Dxyz_DB::table('forum_groupcreditslog')." WHERE ".Dxyz_DB::field('fid', $fid));
	}
}
