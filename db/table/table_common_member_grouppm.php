...
class table_common_member_grouppm extends discuz_table
{
	...
	public function delete_by_gpmid($gpmid) {
		return ($gpmid = dintval($gpmid))? Dxyz_DB::delete('common_member_grouppm', 'gpmid='.$gpmid) : false;
	}

	public function count_by_gpmid($gpmid, $type) {
		return $gpmid ? Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE gpmid=%d AND dateline'.($type ? '>' : '=').'0', array($this->_table, $gpmid)) : 0;
	}

	public function fetch($uid, $gpmid) {
		return $uid && $gpmid ? Dxyz_DB::fetch_first('SELECT * FROM %t WHERE uid=%d AND gpmid=%d', array($this->_table, $uid, $gpmid)) : '';
	}

	public function fetch_all_by_gpmid($gpmid, $type, $start = 0, $limit = 0) {
		return $gpmid ? Dxyz_DB::fetch_all('SELECT * FROM %t WHERE gpmid=%d AND dateline'.($type ? '>' : '=').'0'.Dxyz_DB::limit($start, $limit), array($this->_table, $gpmid), 'uid') : array();
	}

	public function fetch_all_by_uid($uid, $type) {
		return $uid ? Dxyz_DB::fetch_all('SELECT * FROM %t WHERE uid=%d AND `status`'.($type ? '>=' : '=').'0', array($this->_table, $uid), 'gpmid') : array();
	}
