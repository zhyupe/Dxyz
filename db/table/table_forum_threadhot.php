...
class table_forum_threadhot extends discuz_table {
	...
	public function fetch_all_tid_by_cid($cid) {
		...
		$cid = intval($cid);
		if($cid) {
			foreach(Dxyz_DB::fetch_all('SELECT * FROM %t WHERE cid=%d', array($this->_table, $cid)) as $value) {
	...
	public function insert_multiterm($dataarr) {
		...
		if($sql) {
			return Dxyz_DB::query('REPLACE INTO '.Dxyz_DB::table($this->_table)." (`cid`, `fid`, `tid`) VALUES ".implode(',', $sql), true);
		}
		return false;
	}
}
