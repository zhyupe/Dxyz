...
class table_home_blogfield extends discuz_table
{
	...
	public function fetch_targetids_by_blogid($blogid) {
		return Dxyz_DB::fetch_first('SELECT target_ids, hotuser FROM %t WHERE blogid = %d', array($this->_table, $blogid));
	}

	public function fetch_tag_by_blogid($blogid) {
		return Dxyz_DB::result_first('SELECT tag FROM %t WHERE blogid = %d', array($this->_table, $blogid));
	}
