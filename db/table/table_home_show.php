...
class table_home_show extends discuz_table
{
	...
	public function count_by_credit($unitprice = false) {
		...
		return Dxyz_DB::result_first("SELECT COUNT(*) FROM %t WHERE credit>0 {$sql}", $args);
	}

	public function fetch_by_uid_credit($uid) {
		return Dxyz_DB::fetch_first('SELECT unitprice, credit FROM %t WHERE uid=%d AND credit>0', array($this->_table, $uid));
	}
	...
	public function fetch_all_by_unitprice($start, $perpage, $selectall = false) {
		...
		return Dxyz_DB::fetch_all("SELECT {$selectfields} FROM %t ORDER BY unitprice DESC, credit DESC %i", array($this->_table, Dxyz_DB::limit($start, $perpage)));
	}

	public function fetch_all_by_credit($start, $perpage) {
		return Dxyz_DB::fetch_all('SELECT * FROM %t ORDER BY credit DESC %i', array($this->_table, Dxyz_DB::limit($start, $perpage)));
	}

	public function delete_by_credit($credit = 1) {
		return Dxyz_DB::query('DELETE FROM %t WHERE %i', array($this->_table, Dxyz_DB::field('credit', intval($credit), '<')));
	}
	...
	public function update_credit_by_uid($uid, $inc_credit, $limit_credit = true, $unitprice = false, $note = false) {
		...
		$args[] = $uid;
		return Dxyz_DB::query("UPDATE %t SET credit=credit+'%d' {$set_sql} WHERE uid=%d {$sql}", $args);
	}
