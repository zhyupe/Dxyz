...
class table_home_follow_feed extends discuz_table
{
	...
	public function fetch_all_by_uid($uids = 0, $archiver = false, $start = 0, $limit = 0) {
		...
		$query = Dxyz_DB::query("SELECT * FROM %t $wheresql ORDER BY dateline DESC ".Dxyz_DB::limit($start, $limit), $parameter);
	...
	public function fetch_all_by_dateline($dateline, $glue = '>=') {
		...
		return Dxyz_DB::fetch_all("SELECT * FROM %t WHERE dateline{$glue}%d ORDER BY dateline", array($this->_table, $dateline), $this->_pk);
	}

	public function fetch_by_feedid($feedid, $archiver = false) {
		return Dxyz_DB::fetch_first("SELECT * FROM %t WHERE feedid=%d", array($archiver ? $this->_archiver_table : $this->_table, $feedid));
	}

	public function count_by_uid_tid($uid, $tid, $archiver = false) {
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE uid=%d AND tid=%d', array($archiver ? $this->_archiver_table : $this->_table, $uid, $tid));
	}
	...
	public function insert_archiver($data) {
		if(!empty($data) && is_array($data)) {
			return Dxyz_DB::insert($this->_archiver_table, $data, false, true);
		}
		return 0;
	}
	...
	public function delete_by_feedid($feedid, $archiver = false) {
		...
		if($feedid) {
			return Dxyz_DB::delete($archiver ? $this->_archiver_table : $this->_table, Dxyz_DB::field('feedid', $feedid));
		}
		return 0;
	}
	...
	public function delete_by_uid($uids) {
		...
		$delnum = 0;
		if($uids) {
			$delnum = Dxyz_DB::delete($this->_table, Dxyz_DB::field('uid', $uids));
			$delnum += Dxyz_DB::delete($this->_archiver_table, Dxyz_DB::field('uid', $uids));
		}
		return $delnum;
	}
