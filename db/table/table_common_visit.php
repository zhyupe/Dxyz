...
class table_common_visit extends discuz_table
{
	...
	public function inc($ip, $viewadd = 1) {
		return Dxyz_DB::query('UPDATE %t SET view=view+(%d) WHERE `ip`=%s', array($this->_table, $viewadd, $ip));

	}

	public function range($start = 0, $limit = 0) {
		return Dxyz_DB::fetch_all('SELECT * FROM '.Dxyz_DB::table($this->_table).' ORDER BY view DESC'.Dxyz_DB::limit($start, $limit), $this->_pk);
	}
}
