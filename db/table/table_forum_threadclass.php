...
class table_forum_threadclass extends discuz_table
{
	...
	public function fetch_by_fid_name($fid, $name) {
		return Dxyz_DB::fetch_first('SELECT * FROM %t WHERE fid=%d AND name=%s', array($this->_table, $fid, $name));
	}
	public function fetch_all_by_typeid($typeids) {
		$typeids = dintval($typeids, is_array($typeids) ? true : false);
		if($typeids) {
			return Dxyz_DB::fetch_all('SELECT * FROM %t WHERE typeid IN(%n) ORDER BY displayorder', array($this->_table, $typeids), $this->_pk);
		}
		return array();
	}
	public function fetch_all_by_fid($fid) {
		return Dxyz_DB::fetch_all('SELECT * FROM %t WHERE fid=%d ORDER BY displayorder', array($this->_table, $fid), $this->_pk);
	}
	public function fetch_all_by_typeid_fid($typeid, $fid) {
		...
		return Dxyz_DB::fetch_all('SELECT * FROM %t WHERE '.$wheresql, $parameter, $this->_pk);
	}
	public function update_by_fid($fid, $data) {
		...
		if(!empty($data) && is_array($data)) {
			return Dxyz_DB::update($this->_table, $data, Dxyz_DB::field('fid', $fid));
		}
		return 0;
	}
	public function update_by_typeid($typeid, $data) {
		...
		if(!empty($data) && is_array($data)) {
			return Dxyz_DB::update($this->_table, $data, Dxyz_DB::field('typeid', $typeid));
		}
		return 0;
	}
	public function update_by_typeid_fid($typeid, $fid, $data) {
		...
		if(!empty($data) && is_array($data)) {
			return Dxyz_DB::update($this->_table, $data, Dxyz_DB::field('typeid', $typeid).' AND '.Dxyz_DB::field('fid', $fid));
		}
		return 0;
	}
	public function delete_by_typeid($typeid) {
		...
		return Dxyz_DB::delete($this->_table, Dxyz_DB::field('typeid', $typeid));
	}
	public function delete_by_typeid_fid($typeid, $fid) {
		...
		return Dxyz_DB::delete($this->_table, Dxyz_DB::field('typeid', $typeid).' AND '.Dxyz_DB::field('fid', $fid));
	}
	public function count_by_fid($fid) {
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE fid=%d', array($this->_table, $fid));
	}
