...
class table_common_member extends discuz_table_archive
{
	...
	public function update_credits($uid, $credits) {
		if($uid) {
			...
			Dxyz_DB::update($this->_table, $data, array('uid' => intval($uid)), 'UNBUFFERED');
	...
	public function update_by_groupid($groupid, $data) {
		...
		if($groupid && !empty($data) && is_array($data)) {
			Dxyz_DB::update($this->_table, $data, Dxyz_DB::field('groupid', $groupid), 'UNBUFFERED');
		}
		if($uids) {
	...
	public function increase($uids, $setarr) {
		...
		if(!empty($sql)){
			Dxyz_DB::query("UPDATE ".Dxyz_DB::table($this->_table)." SET ".implode(',', $sql)." WHERE uid IN (".dimplode($uids).")", 'UNBUFFERED');
	...
	public function fetch_all_groupid() {
		return Dxyz_DB::fetch_all('SELECT DISTINCT(groupid) FROM '.Dxyz_DB::table($this->_table), null, 'groupid');
	}

	public function fetch_all_by_allowadmincp($val, $glue = '=') {
		return Dxyz_DB::fetch_all('SELECT * FROM '.Dxyz_DB::table($this->_table).' WHERE '.Dxyz_DB::field('allowadmincp', intval($val), $glue), NULL, 'uid');
	}

	public function update_admincp_manage($uids) {
		if(($uids = dintval($uids, true))) {
			$data = Dxyz_DB::query('UPDATE '.Dxyz_DB::table($this->_table).' SET allowadmincp=allowadmincp | 1 WHERE uid IN ('.dimplode($uids).')');
	...
	public function clean_admincp_manage($uids) {
		if(($uids = dintval($uids, true))) {
			$data = Dxyz_DB::query('UPDATE '.Dxyz_DB::table($this->_table).' SET allowadmincp=allowadmincp & 0xFE WHERE uid IN ('.dimplode($uids).')');
			$this->reset_cache($uids);
			return $data;
		}
		return false;
	...
	public function fetch_all_ban_by_groupexpiry($timestamp) {
		return ($timestamp = intval($timestamp)) ? Dxyz_DB::fetch_all("SELECT uid, groupid, credits FROM ".Dxyz_DB::table($this->_table)." WHERE groupid IN ('4', '5') AND groupexpiry>'0' AND groupexpiry<'$timestamp'", array(), 'uid') : array();
	}
	...
	public function fetch_all_by_like_username($username, $start = 0, $limit = 0) {
		$data = array();
		if($username) {
			$data = Dxyz_DB::fetch_all('SELECT * FROM %t WHERE username LIKE %s'.Dxyz_DB::limit($start, $limit), array($this->_table, stripsearchkey($username).'%'), 'uid');
		}
		return $data;
	}

	public function count_by_like_username($username) {
		return !empty($username) ? Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE username LIKE %s', array($this->_table, stripsearchkey($username).'%')) : 0;
	}


	public function fetch_runtime() {
		return Dxyz_DB::result_first("SELECT (MAX(regdate)-MIN(regdate))/86400 AS runtime FROM ".Dxyz_DB::table($this->_table));
	}

	public function count_admins() {
		return Dxyz_DB::result_first("SELECT COUNT(*) FROM ".Dxyz_DB::table($this->_table)." WHERE adminid<>'0' AND adminid<>'-1'");
	}

	public function count_by_regdate($timestamp) {
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE regdate>%d', array($this->_table, $timestamp));
	}
	...
	public function fetch_all_stat_memberlist($username, $orderby = '', $sort = '', $start = 0, $limit =  0) {
		...
		$memberlist = array();
		$query = Dxyz_DB::query("SELECT m.uid, m.username, mp.gender, m.email, m.regdate, ms.lastvisit, mc.posts, m.credits
			FROM ".Dxyz_DB::table($this->_table)." m
			LEFT JOIN ".Dxyz_DB::table('common_member_profile')." mp ON mp.uid=m.uid
			LEFT JOIN ".Dxyz_DB::table('common_member_status')." ms ON ms.uid=m.uid
			LEFT JOIN ".Dxyz_DB::table('common_member_count')." mc ON mc.uid=m.uid
			$sql ORDER BY ".Dxyz_DB::order($orderby, $sort).Dxyz_DB::limit($start, $limit));
		while($member = Dxyz_DB::fetch($query)) {
	...
	public function count_zombie() {
		...
		return Dxyz_DB::result_first('SELECT count(*) FROM %t mc, %t ms WHERE mc.posts<5 AND ms.lastvisit<%d AND ms.uid=mc.uid', array('common_member_count', 'common_member_status', $dateline));
	}
	...
	public function split($splitnum, $iscron = false) {
		...
		$dateline = TIMESTAMP - 7776000;//60*60*24*90
		$temptablename = Dxyz_DB::table('common_member_temp___');
		if(!Dxyz_DB::fetch_first("SHOW TABLES LIKE '$temptablename'")) {
			Dxyz_DB::query("CREATE TABLE $temptablename (`uid` int(10) NOT NULL DEFAULT 0,PRIMARY KEY (`uid`)) ENGINE=MYISAM;");
		}
		$splitnum = max(1, intval($splitnum));
		if(!Dxyz_DB::result_first('SELECT COUNT(*) FROM '.$temptablename)) {
			Dxyz_DB::query('INSERT INTO '.$temptablename.' (`uid`) SELECT ms.uid AS uid FROM %t mc, %t ms WHERE mc.posts<5 AND ms.lastvisit<%d AND mc.uid=ms.uid ORDER BY ms.lastvisit LIMIT %d', array('common_member_count', 'common_member_status', $dateline, $splitnum));
		}
		...
		if(Dxyz_DB::result_first('SELECT COUNT(*) FROM '.$temptablename)) {
			...
			$uidlist = Dxyz_DB::fetch_all('SELECT uid FROM '.$temptablename, null, 'uid');
			...
			if(Dxyz_DB::query($movesql, array('common_member_archive', 'common_member'), false, true)) {
				Dxyz_DB::query($deletesql, array('common_member'), false, true);
			}
			if(Dxyz_DB::query($movesql, array('common_member_profile_archive', 'common_member_profile'), false, true)) {
				Dxyz_DB::query($deletesql, array('common_member_profile'), false, true);
			}
			if(Dxyz_DB::query($movesql, array('common_member_field_forum_archive', 'common_member_field_forum'), false, true)) {
				Dxyz_DB::query($deletesql, array('common_member_field_forum'), false, true);
			}
			if(Dxyz_DB::query($movesql, array('common_member_field_home_archive', 'common_member_field_home'), false, true)) {
				Dxyz_DB::query($deletesql, array('common_member_field_home'), false, true);
			}
			if(Dxyz_DB::query($movesql, array('common_member_status_archive', 'common_member_status'), false, true)) {
				Dxyz_DB::query($deletesql, array('common_member_status'), false, true);
			}
			if(Dxyz_DB::query($movesql, array('common_member_count_archive', 'common_member_count'), false, true)) {
				Dxyz_DB::query($deletesql, array('common_member_count'), false, true);
			}

			Dxyz_DB::query('DROP TABLE '.$temptablename);
	...
	public function count_by_credits($credits) {
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE credits>%d', array($this->_table, $credits));
	}
	...
	public function fetch_all_for_spacecp_search($wherearr, $fromarr, $start = 0, $limit = 100) {
		...
		if(!$fromarr) {
			$fromarr[] = Dxyz_DB::table($this->_table);
		}
		return Dxyz_DB::fetch_all("SELECT s.* FROM ".implode(',', $fromarr)." WHERE ".implode(' AND ', $wherearr).Dxyz_DB::limit($start, $limit));
	}

	public function fetch_all_girls_for_ranklist($offset = 0, $limit = 20, $orderby = 'ORDER BY s.unitprice DESC, s.credit DESC') {
		$members = array();
		$query = Dxyz_DB::query("SELECT m.uid, m.username, mc.*, mp.gender
			FROM ".Dxyz_DB::table('common_member')." m
			LEFT JOIN ".Dxyz_DB::table('home_show')." s ON s.uid=m.uid
			LEFT JOIN ".Dxyz_DB::table('common_member_profile')." mp ON mp.uid=m.uid
			LEFT JOIN ".Dxyz_DB::table('common_member_count')." mc ON mc.uid=m.uid
			WHERE mp.gender='2'
			ORDER BY $orderby
	...
	public function fetch_all_order_by_credit_for_ranklist($num, $orderby) {
		...
		if($orderby === 'all') {
			$sql = "SELECT m.uid,m.username,m.videophotostatus,m.groupid,m.credits,field.spacenote FROM ".Dxyz_DB::table('common_member')." m
				LEFT JOIN ".Dxyz_DB::table('common_member_field_home')." field ON field.uid=m.uid
			...
			$sql = "SELECT m.uid,m.username,m.videophotostatus,m.groupid, mc.extcredits$orderby AS extcredits
				FROM ".Dxyz_DB::table('common_member')." m
				LEFT JOIN ".Dxyz_DB::table('common_member_count')." mc ON mc.uid=m.uid WHERE mc.extcredits$orderby>0
				ORDER BY extcredits$orderby DESC LIMIT 0, $num";
		}

		$query = Dxyz_DB::query($sql);
		while($result = Dxyz_DB::fetch($query)) {
			$data[] = $result;
		}
	...
	public function fetch_all_order_by_friendnum_for_ranklist($num) {
		...
		$query = Dxyz_DB::query('SELECT uid, friends FROM '.Dxyz_DB::table('common_member_count').' WHERE friends>0 ORDER BY friends DESC LIMIT '.$num);
		while($user = Dxyz_DB::fetch($query)) {
			$users[$user['uid']] = $user;
			$oldorder[] = $user['uid'];
		}
		$uids = array_keys($users);
		if($uids) {
			$query = Dxyz_DB::query('SELECT m.uid, m.username, m.videophotostatus, m.groupid, field.spacenote
				FROM '.Dxyz_DB::table('common_member')." m
				LEFT JOIN ".Dxyz_DB::table('common_member_field_home')." field ON m.uid=field.uid
	...
	public function max_uid() {
		return Dxyz_DB::result_first('SELECT MAX(uid) FROM %t', array($this->_table));
	}
