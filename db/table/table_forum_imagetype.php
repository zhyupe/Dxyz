...
class table_forum_imagetype extends discuz_table
{
	...
	public function fetch_all_by_type($type, $available = null) {
		...
		return Dxyz_DB::fetch_all("SELECT * FROM %t WHERE type=%s %i ORDER BY displayorder", array($this->_table, $type, $available));
	}

	public function fetch_all_available() {
		return Dxyz_DB::fetch_all("SELECT * FROM %t WHERE available=1", array($this->_table));
	}

	public function count_by_name($type, $name) {
		return Dxyz_DB::result_first("SELECT COUNT(*) FROM %t WHERE type=%s AND name=%s", array($this->_table, $type, $name));
	}
