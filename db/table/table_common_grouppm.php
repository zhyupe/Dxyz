...
class table_common_grouppm extends discuz_table
{
	...
	public function fetch_all_by_id_authorid($id = 0, $authorid = 0) {
		...
		$query = Dxyz_DB::query("SELECT * FROM %t $wheresql ORDER BY id DESC", $parameter);
	...
	public function count_by_id_authorid($id, $authorid) {
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE id=%d AND authorid=%d', array($this->_table, $id, $authorid));
	}
