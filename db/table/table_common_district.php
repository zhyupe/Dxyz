...
class table_common_district extends discuz_table
{
	...
	public function fetch_all_by_upid($upid, $order = null, $sort = 'DESC') {
		...
		if($upid !== null) {
			$ordersql = $order !== null && !empty($order) ? ' ORDER BY '.Dxyz_DB::order($order, $sort) : '';
			return Dxyz_DB::fetch_all('SELECT * FROM %t WHERE '.Dxyz_DB::field('upid', $upid)." $ordersql", array($this->_table), $this->_pk);
		}
		return array();
	}
	...
	public function fetch_all_by_name($name) {
		if(!empty($name)) {
			return Dxyz_DB::fetch_all('SELECT * FROM %t WHERE '.Dxyz_DB::field('name', $name), array($this->_table));
		}
		return array();
	}
