...
class table_home_follow extends discuz_table
{
	...
	public function fetch_all_following_by_uid($uid, $status = 0, $start = 0, $limit = 0) {
		...
		if((!$force && ($data = $this->fetch_cache($uid)) === false) || $force) {
			...
			$data = Dxyz_DB::fetch_all("SELECT * FROM %t $wheresql ORDER BY dateline DESC ".Dxyz_DB::limit($start, $limit), $parameter, 'followuid');
			if(!$force) {
	...
	public function fetch_all_follower_by_uid($uids, $start = 0, $limit = 0) {
		...
		if($uids) {
			...
			return Dxyz_DB::fetch_all("SELECT * FROM %t WHERE $fsql ORDER BY dateline DESC ".Dxyz_DB::limit($start, $limit), $parameter, 'uid');
		}
		return array();
	}
	...
	public function fetch_all_by_uid_followuid($uid, $followuids) {
		...
		if($followuids) {
			return Dxyz_DB::fetch_all("SELECT * FROM %t WHERE uid=%d AND followuid IN(%n)", array($this->_table, $uid, $followuids), 'followuid');
		}
		return array();
	}
	public function fetch_status_by_uid_followuid($uid, $followuid) {
		return Dxyz_DB::fetch_all('SELECT * FROM %t WHERE (uid=%d AND followuid=%d) OR (uid=%d AND followuid=%d)', array($this->_table, $uid, $followuid, $followuid, $uid), 'uid');
	}
	...
	public function fetch_all_by_uid_fusername($uid, $users) {
		...
		return Dxyz_DB::fetch_all('SELECT * FROM %t WHERE uid=%d AND fusername IN(%n)', array($this->_table, $uid, $users));
	}
	...
	public function count_follow_user($uid, $type = 0, $dateline = 0) {
		...
		$wheresql = !empty($wherearr) && is_array($wherearr) ? ' WHERE '.implode(' AND ', $wherearr) : '';
		$count = Dxyz_DB::result_first("SELECT COUNT(*) FROM %t $wheresql", $parameter);
		return $count;
	}
	...
	public function fetch_by_uid_followuid($uid, $followuid) {
		return Dxyz_DB::fetch_first("SELECT * FROM %t WHERE uid=%d AND followuid=%d", array($this->_table, $uid, $followuid));
	}
	public function update_by_uid_followuid($uid, $followuid, $data) {
		...
		if(!empty($data) && is_array($data) && $uid && $followuid) {
			...
			return Dxyz_DB::update($this->_table, $data, Dxyz_DB::field('uid', $uid).' AND '.Dxyz_DB::field('followuid', $followuid));
		}
		return 0;
	}
	...
	public function delete_by_uid_followuid($uid, $followuid) {
		$this->clear_cache($uid);
		return Dxyz_DB::query('DELETE FROM %t WHERE uid=%d AND followuid=%d', array($this->_table, $uid, $followuid));
	}
}
