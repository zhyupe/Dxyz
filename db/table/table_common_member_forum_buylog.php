...
class table_common_member_forum_buylog extends discuz_table
{
	...
	public function get_credits($uid, $fid) {
		...
		if(!$credits) {
			$credits = Dxyz_DB::result_first('SELECT credits FROM %t WHERE uid=%d AND fid=%d', array($this->_table, $uid, $fid));
	...
	public function delete_by_uid($uids) {
		Dxyz_DB::delete($this->_table, Dxyz_DB::field('uid', $uids));
	}

	public function delete_by_fid($fids) {
		Dxyz_DB::delete($this->_table, Dxyz_DB::field('fid', $fids));
	}
}
