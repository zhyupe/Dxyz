...
class table_forum_sofa extends discuz_table
{
	...
	public function range($start = 0, $limit = 20) {
		return Dxyz_DB::fetch_all('SELECT * FROM %t ORDER BY tid DESC %i', array($this->_table, Dxyz_DB::limit($start, $limit)), $this->_pk);
	}

	public function fetch_all_by_fid($fid, $start = 0, $limit = 20) {
		$fid = dintval($fid, true);
		return Dxyz_DB::fetch_all('SELECT * FROM %t WHERE fid=%d ORDER BY tid DESC %i', array($this->_table, $fid, Dxyz_DB::limit($start, $limit)), $this->_pk);
	}
