...
class table_forum_thread extends discuz_table
{
	...
	public function fetch($tid, $tableid = 0) {
		...
		if($tid && ($data = $this->fetch_cache($tid)) === false) {
			...
			$data = Dxyz_DB::fetch_first("SELECT * FROM %t WHERE tid=%d", $parameter);
	...
	public function fetch_by_fid_displayorder($fid, $displayorder = 0, $glue = '>=', $order = 'lastpost', $sort = 'DESC') {
		...
		if(!empty($fid)) {
			...
			$ordersql = !empty($order) ? ' ORDER BY '.Dxyz_DB::order($order, $sort) : '';
			return Dxyz_DB::fetch_first("SELECT * FROM %t WHERE fid=%d AND displayorder{$glue}%d $ordersql ".Dxyz_DB::limit(0, 1), $parameter);
		}
		return array();
	}
	public function fetch_next_tid_by_fid_lastpost($fid, $lastpost, $glue = '>', $sort = 'DESC', $tableid = 0) {
		$glue = helper_util::check_glue($glue);
		return Dxyz_DB::result_first("SELECT tid FROM %t WHERE fid=%d AND displayorder>=0 AND closed=0 AND lastpost{$glue}%d  ORDER BY ".Dxyz_DB::order('lastpost', $sort).Dxyz_DB::limit(1), array($this->get_table_name($tableid), $fid, $lastpost));
	}
	...
	public function fetch_all_by_digest_displayorder($digest, $digestglue = '=', $displayorder = 0, $glue = '>=', $start = 0, $limit = 0, $tableid = 0) {
		...
		$glue = helper_util::check_glue($glue);
		return Dxyz_DB::fetch_all("SELECT * FROM %t WHERE digest{$digestglue}%d AND displayorder{$glue}%d".Dxyz_DB::limit($start, $limit), $parameter, $this->_pk);
	}
	...
	public function fetch_all_by_fid_typeid_displayorder($fid, $typeid = null, $displayorder = null, $glue = '=', $start = 0, $limit = 0) {
		...
		return Dxyz_DB::fetch_all("SELECT * FROM %t $wheresql ORDER BY lastpost DESC ".Dxyz_DB::limit($start, $limit), $parameter, $this->_pk);
	}
	public function fetch_all_by_fid_lastpost($fid, $lstart = 0, $lend = 0, $tableid = 0) {
		...
		$wheresql = !empty($wherearr) && is_array($wherearr) ? ' WHERE '.implode(' AND ', $wherearr) : '';
		return Dxyz_DB::fetch_all("SELECT * FROM %t $wheresql ORDER BY lastpost DESC ".Dxyz_DB::limit(0, 100), $parameter, $this->_pk);
	}
	...
	public function fetch_all_by_authorid_displayorder($authorid, $displayorder = null, $dglue = '=', $closed = null, $subject = '', $start = 0, $limit = 0, $replies = null, $fid = null, $rglue = '>=', $tableid = 0) {
		...
		$wheresql = !empty($wherearr) && is_array($wherearr) ? ' WHERE '.implode(' AND ', $wherearr) : '';
		return Dxyz_DB::fetch_all("SELECT * FROM %t $wheresql ORDER BY dateline DESC ".Dxyz_DB::limit($start, $limit), $parameter, $this->_pk);
	}
	...
	public function fetch_all_by_tid($tids, $start = 0, $limit = 0, $tableid = 0) {
		...
		if(($data = $this->fetch_cache($tids)) === false || count($tids) != count($data)) {
			...
			if(!empty($tids)) {
				...
				$query = Dxyz_DB::query("SELECT * FROM %t WHERE tid IN(%n)".Dxyz_DB::limit($start, $limit), $parameter);
	...
	public function fetch_all_by_tid_fid_displayorder($tids, $fids = null, $displayorder = null, $order = 'dateline', $start = 0, $limit = 0, $glue = '>=', $sort = 'DESC', $tableid = 0) {
		...
		if($order) {
			$order = 'ORDER BY '.Dxyz_DB::order($order, $sort);
		}
		if(!empty($wherearr)) {
			$wheresql = !empty($wherearr) && is_array($wherearr) ? ' WHERE '.implode(' AND ', $wherearr) : '';
			return Dxyz_DB::fetch_all("SELECT * FROM %t $wheresql $order ".Dxyz_DB::limit($start, $limit), $parameter, $this->_pk);
		} else {
			return array();
		}
	}
	...
	public function fetch_all_by_tid_or_fid($fid, $tids = array()) {
		...
		return Dxyz_DB::fetch_all("SELECT * FROM %t WHERE fid=%d AND displayorder=1 $forumstickytids ORDER BY lastpost DESC", $parameter);
	}
	...
	public function fetch_all_by_authorid($authorid, $start = 0, $limit = 0, $tableid = 0) {
		$authorid = dintval($authorid, true);
		return Dxyz_DB::fetch_all("SELECT * FROM %t WHERE %i ORDER BY dateline DESC ".Dxyz_DB::limit($start, $limit), array($this->get_table_name($tableid), Dxyz_DB::field('authorid', $authorid)), $this->_pk);
	}
	...
	public function fetch_all_by_dateline($starttime, $start = 0, $limit = 0, $order = 'dateline', $sort = 'DESC') {
		if($starttime) {
			$orderby = '';
			if(!empty($order)) {
				$orderby = "ORDER BY ".Dxyz_DB::order($order, $sort);
			}
			$parameter = array($this->get_table_name(), $starttime);
			return Dxyz_DB::fetch_all("SELECT * FROM %t WHERE dateline>=%d AND displayorder>'-1' $orderby ".Dxyz_DB::limit($start, $limit), $parameter, $this->_pk);
		}
		return array();
	}
	...
	public function fetch_all_by_fid_displayorder($fids, $displayorder = null, $dateline = null, $recommends = null, $start = 0, $limit = 0, $order = 'dateline', $sort = 'DESC', $dglue = '>=') {
		...
		$ordersql = !empty($order) ? ' ORDER BY '.Dxyz_DB::order($order, $sort) : '';
		$wheresql = !empty($wherearr) && is_array($wherearr) ? ' WHERE '.implode(' AND ', $wherearr) : '';
		return Dxyz_DB::fetch_all("SELECT * FROM %t $wheresql $ordersql ".Dxyz_DB::limit($start, $limit), $parameter, $this->_pk);
	}

	public function fetch_all_new_thread_by_tid($tid = 0, $start = 0, $limit = 0, $tableid = 0, $glue = '>', $sort = 'ASC') {
		$glue = helper_util::check_glue($glue);
		return Dxyz_DB::fetch_all("SELECT * FROM %t WHERE tid{$glue}%d ORDER BY ".Dxyz_DB::order('tid', $sort).Dxyz_DB::limit($start, $limit), array($this->get_table_name($tableid), $tid), $this->_pk);
	}
	public function fetch_all_group_thread_by_fid_displayorder($fids, $displayorder = null, $dateline = null, $lastpost = null, $digest = null, $order = 'dateline', $start = 0, $limit = 0, $dglue = '>=') {
		...
		$ordersql = !empty($order) ? 'ORDER BY'.Dxyz_DB::order($order, 'DESC') : '';
		$wheresql = !empty($wherearr) && is_array($wherearr) ? ' WHERE '.implode(' AND ', $wherearr) : '';
		return Dxyz_DB::fetch_all("SELECT * FROM %t $wheresql $ordersql ".Dxyz_DB::limit($start, $limit), $parameter, $this->_pk);
	}
	...
	public function fetch_all_by_fid($fids, $start = 0, $limit = 0, $tableid = 0) {
		$fids = dintval($fids, true);
		if($fids) {
			return Dxyz_DB::fetch_all("SELECT * FROM %t WHERE fid IN(%n) ".Dxyz_DB::limit($start, $limit), array($this->get_table_name($tableid), (array)$fids));
		}
		return array();
	}
	...
	public function fetch_all_by_replies($number, $start = 0, $limit = 0, $glue = '>', $tableid = 0) {
		...
		if($number) {
			$glue = helper_util::check_glue($glue);
			return Dxyz_DB::fetch_all("SELECT * FROM %t WHERE replies{$glue}%d ".Dxyz_DB::limit($start, $limit), array($this->get_table_name($tableid), $number));
		}
		return array();
	}
	...
	public function fetch_all_rank_thread($dateline, $notfid, $order = 'dateline', $start = 0, $limit = 0) {
		...
		$wheresql = ' WHERE '.implode(' AND ', $wherearr);
		$ordersql = !empty($order) ? ' ORDER BY '.Dxyz_DB::order($order, 'DESC') : '';
		$query = Dxyz_DB::query("SELECT tid, fid, author, authorid, subject, dateline, views, replies, favtimes, sharetimes, heats FROM %t $wheresql $ordersql ".Dxyz_DB::limit($start, $limit), $parameter);
		while($value = Dxyz_DB::fetch($query)) {
			$data[$value['tid']] = $value;
	...
	public function fetch_all_rank_poll($dateline, $notfid, $order = 'dateline', $start = 0, $limit = 0) {
		...
		$wheresql = ' WHERE '.implode(' AND ', $wherearr);
		$ordersql = !empty($order) ? ' ORDER BY '.Dxyz_DB::order($order, 'DESC') : '';
		return Dxyz_DB::fetch_all("SELECT t.tid, t.fid, t.author, t.authorid, t.subject, t.dateline, t.favtimes, t.sharetimes, t.heats,  p.pollpreview, p.voters FROM %t t LEFT JOIN %t p ON p.tid=t.tid $wheresql $ordersql ".Dxyz_DB::limit($start, $limit), $parameter, $this->_pk);
	}
	...
	public function fetch_all_by_recyclebine($fid = 0, $isgroup = 0, $author = array(), $username = array(), $pstarttime = 0, $pendtime = 0, $mstarttime = 0, $mendtime = 0, $keywords = '', $start = 0, $limit = 0) {
		...
		return Dxyz_DB::fetch_all('SELECT f.name AS forumname, f.allowsmilies, f.allowhtml, f.allowbbcode, f.allowimgcode,
				...
				FROM '.Dxyz_DB::table('forum_thread').' t LEFT JOIN '.Dxyz_DB::table('forum_threadmod').' tm ON tm.tid=t.tid
				LEFT JOIN '.Dxyz_DB::table('forum_forum').' f ON f.fid=t.fid '.$sql[0].' ORDER BY t.dateline DESC '.Dxyz_DB::limit($start, $limit), $sql[1]);
	}
	...
	public function fetch_all_moderate($fid = 0, $displayorder = null, $isgroup = null, $dateline = null, $author = null, $subject = null) {
		...
		return Dxyz_DB::fetch_all('SELECT * FROM %t '.$parameter[0], $parameter[1], $this->_pk);
	}

	public function fetch_all_movedthread($start = 0, $limit = 0) {
		return Dxyz_DB::fetch_all('SELECT t1.tid, t2.tid AS threadexists, f.status, t1.isgroup FROM %t t1
				LEFT JOIN %t t2 ON t2.tid=t1.closed AND t2.displayorder>=0 LEFT JOIN %t f ON f.fid=t1.fid
				WHERE t1.closed>1'.Dxyz_DB::limit($start, $limit), array($this->get_table_name(), $this->get_table_name(), 'forum_forum'));
	}
	...
	public function fetch_all_by_fid_cover_lastpost($fid, $cover = null, $starttime = 0, $endtime = 0, $start = 0, $limit = 0) {
		...
		$wheresql = ' WHERE '.implode(' AND ', $wherearr);
		return Dxyz_DB::fetch_all('SELECT * FROM %t '.$wheresql.Dxyz_DB::limit($start, $limit), $parameter, $this->_pk);
	}
	public function fetch_all_by_posttableid_displayorder($tableid = 0, $posttableid = 0, $displayorder = 0) {
		return Dxyz_DB::fetch_all('SELECT * FROM %t WHERE posttableid=%d AND displayorder>=%d ORDER BY lastpost'.Dxyz_DB::limit(1000), array($this->get_table_name($tableid), $posttableid, $displayorder), $this->_pk);
	}
	...
	public function fetch_all_search($conditions, $tableid = 0, $start = 0, $limit = 0, $order = '', $sort = 'DESC', $forceindex='') {
		...
		$data = Dxyz_DB::fetch_all("SELECT * FROM ".Dxyz_DB::table($this->get_table_name($tableid))." $forceindex".$this->search_condition($conditions)." $ordersql ".Dxyz_DB::limit($start, $limit));
	...
	public function fetch_all_by_special($special, $authorid = 0, $replies = 0, $displayorder = null, $subject = '', $join = 0, $start = 0, $limit = 0, $order = 'dateline', $sort = 'DESC') {
		...
		$ordersql = !empty($order) ? ' ORDER BY t.'.Dxyz_DB::order($order, $sort) : '';
		return Dxyz_DB::fetch_all("SELECT t.* FROM %t t $condition[jointable] ".$condition['where'].$ordersql.Dxyz_DB::limit($start, $limit), $condition['parameter'], $this->_pk);
	}
	public function fetch_all_heats() {
		$heatdateline = getglobal('timestamp') - 86400 * getglobal('setting/indexhot/days');
		$addtablesql = $addsql = '';
		if(!helper_access::check_module('group')) {
			$addtablesql = " LEFT JOIN ".Dxyz_DB::table('forum_forum')." f ON f.fid = t.fid ";
			$addsql = " AND f.status IN ('0', '1') ";
		}
		return Dxyz_DB::fetch_all("SELECT t.tid,t.posttableid,t.views,t.dateline,t.replies,t.author,t.authorid,t.subject,t.price
				FROM ".Dxyz_DB::table('forum_thread')." t $addtablesql
	...
	public function count_by_special($special, $authorid = 0, $replies = 0, $displayorder = null, $subject = '', $join = 0) {
		$condition = $this->make_special_condition($special, $authorid, $replies, $displayorder, $subject, $join, 0);
		return Dxyz_DB::result_first("SELECT COUNT(*) FROM %t t $condition[jointable] ".$condition['where'], $condition['parameter']);
	}
	private function make_special_condition($special, $authorid = 0, $replies = 0, $displayorder = null, $subject = '', $join = 0, $tableid = 0) {
		...
		if($join) {
			if($special == 1) {
				...
				$condition['jointable'] = ', '.Dxyz_DB::table('forum_pollvoter').' p ';
			} elseif($special == 5) {
				$parameter[] = $authorid;
				...
				$condition['jointable'] = ', '.Dxyz_DB::table($posttable).' p ';
			}
		}
	...
	public function count_search($conditions, $tableid = 0, $prefix = false) {
		$prefix = $prefix ? '' : 't';
		return Dxyz_DB::result_first("SELECT COUNT(*) FROM %t $prefix %i", array($this->get_table_name($tableid), $this->search_condition($conditions, $prefix)));
	}
	...
	public function search_condition($conditions, $prefix = false) {
		...
		if($conditions['rate'] == 1) {
			$wherearr[] = $prefix.Dxyz_DB::field('rate', 0, '>');
		...
		if($conditions['highlight'] == 1) {
			$wherearr[] = $prefix.Dxyz_DB::field('highlight', 0, '>');
		...
		if(!empty($conditions['special'])) {
			...
			if($conditions['specialthread'] == 1) {
				$wherearr[] = $prefix.Dxyz_DB::field('special', $conditions['special']);
				$this->_urlparam[] = "specialthread=1";
			} elseif($conditions['specialthread'] == 2) {
				$wherearr[] = $prefix.Dxyz_DB::field('special', $conditions['special'], 'notin');
				$this->_urlparam[] = "specialthread=2";
			}
		}
		if(isset($conditions['isgroup']) && in_array($conditions['isgroup'], array(0, 1))) {
			$wherearr[] = $prefix.Dxyz_DB::field('isgroup', $conditions['isgroup']);
		}
		...
		if(trim($conditions['keywords'])) {
			...
			for($i = 0; $i < count($keywords); $i++) {
				$sqlkeywords .= " $or ".$prefix.Dxyz_DB::field('subject', '%'.$keywords[$i].'%', 'like');
				$or = 'OR';
			}
	...
	public function update_displayorder_by_tid_displayorder($tids, $olddisplayorder, $newdisplayorder) {
		...
		if($tids) {
			return Dxyz_DB::query('UPDATE %t SET displayorder=%d WHERE tid IN (%n) AND displayorder=%d', array($this->get_table_name(), $newdisplayorder, $tids, $olddisplayorder));
		}
		return 0;
	}
	...
	public function update($tid, $data, $unbuffered = false, $low_priority = false, $tableid = 0, $realdata = false) {
		...
		if($data && is_array($data) && $tid) {
			if(!$realdata) {
				$num = Dxyz_DB::update($this->get_table_name($tableid), $data, Dxyz_DB::field('tid', $tid), $unbuffered, $low_priority);
				$this->update_batch_cache((array)$tid, $data);
			} else {
				$num = Dxyz_DB::query('UPDATE '.Dxyz_DB::table($this->get_table_name($tableid))." SET ".implode(',', $data)." WHERE ".Dxyz_DB::field('tid', $tid), 'UNBUFFERED');
	...
	public function update_by_fid($fid, $data, $tableid = 0) {
		...
		if($data && is_array($data) && $fid) {
			return Dxyz_DB::update($this->get_table_name($tableid), $data, Dxyz_DB::field('fid', $fid));
		}
		return array();
	}
	public function update_by_tid_displayorder($tid, $displayorder, $data, $fid = 0, $tableid = 0) {
		$condition = array();
		$tid = dintval($tid, true);
		$condition[] = Dxyz_DB::field('tid', $tid);
		if($fid) {
			$fid = dintval($fid, true);
			$condition[] = Dxyz_DB::field('fid', $fid);
		}
		$condition[] = Dxyz_DB::field('displayorder', $displayorder);
		if($data && is_array($data) && $tid) {
			return Dxyz_DB::update($this->get_table_name($tableid), $data, implode(' AND ', $condition));
		}
		return 0;
	}
	public function update_by_closed($tids, $data, $tableid = 0) {
		...
		if(!empty($data) && is_array($data)) {
			$num = Dxyz_DB::update($this->get_table_name($tableid), $data, Dxyz_DB::field('closed', $tids), true);
			if($num) {
	...
	public function update_status_by_tid($tids, $value, $glue = '|') {
		...
		if($tids) {
			...
			$glue = helper_util::check_glue($glue);
			return Dxyz_DB::query("UPDATE %t SET status=status{$glue}%s WHERE tid IN(%n)", array($this->get_table_name(), $value, (array)$tids));
		}
		return 0;
	}
	...
	public function update_sortid_by_sortid($sortid, $oldsortid) {
		...
		if($oldsortid) {
			return Dxyz_DB::query("UPDATE %t SET sortid=%d WHERE sortid IN (%n)", array($this->get_table_name(), $sortid, $oldsortid));
		}
		return 0;
	}
	...
	public function increase($tids, $fieldarr, $low_priority = false, $tableid = 0, $getsetarr = false) {
		...
		if(!empty($sql)){
			...
			$num = Dxyz_DB::query($cmd.Dxyz_DB::table($this->get_table_name($tableid))." SET ".implode(',', $sql)." WHERE tid IN (".dimplode($tids).")", 'UNBUFFERED');
	...
	public function insert($data, $return_insert_id = false, $replace = false, $silent = false) {
		if($data && is_array($data)) {
			...
			return Dxyz_DB::insert($this->_table, $data, $return_insert_id, $replace, $silent);
		}
		return 0;
	}
	...
	public function count_by_authorid($authorid, $tableid = 0) {
		return Dxyz_DB::result_first("SELECT COUNT(*) FROM %t WHERE authorid=%d", array($this->get_table_name($tableid), $authorid));
	}

	public function count_by_fid($fid, $tableid = 0) {
		return Dxyz_DB::result_first("SELECT COUNT(*) FROM %t WHERE fid=%d", array($this->get_table_name($tableid), $fid));
	}

	public function count_by_displayorder($displayorder) {
		return Dxyz_DB::result_first("SELECT COUNT(*) FROM %t WHERE displayorder=%d", array($this->get_table_name(), $displayorder));
	}

	public function count_by_replies($number, $glue = '>') {
		$glue = helper_util::check_glue($glue);
		return Dxyz_DB::result_first("SELECT COUNT(*) FROM %t WHERE replies{$glue}%d", array($this->get_table_name(), $number));
	}
	...
	public function count_posts_by_fid($fid, $forcetableid = null) {
		...
		foreach($threadtableids as $tableid) {
			$value = Dxyz_DB::fetch_first('SELECT COUNT(*) AS threads, SUM(replies)+COUNT(*) AS posts FROM %t WHERE fid=%d AND displayorder>=0', array($this->get_table_name($tableid), $fid));
	...
	public function count_by_fid_displayorder_authorid($fid, $displayorder, $authorid, $tableid=0) {
		return Dxyz_DB::result_first("SELECT COUNT(*) FROM %t WHERE fid=%d AND displayorder=%d AND authorid=%d", array($this->get_table_name($tableid), $fid, $displayorder, $authorid));
	}
	...
	public function count_by_posttableid_displayorder($tableid = 0, $posttableid = 0, $displayorder = 0) {
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE posttableid=%d AND displayorder=%d', array($this->get_table_name($tableid), $posttableid, $displayorder));
	}

	public function count_by_tableid($tableid) {
		return Dxyz_DB::result_first("SELECT COUNT(*) FROM %t", array($this->get_table_name($tableid)));
	}

	public function count_by_tid_fid($tids, $fids = array(), $isgroup = -1, $author = '', $subject = '') {
		$condition = $this->make_query_condition($tids, $fids, $isgroup, $author, $subject);
		return Dxyz_DB::result_first("SELECT COUNT(*) FROM %t $condition[0]", $condition[1]);
	}


	public function count_group_thread_by_fid($fid) {
		return Dxyz_DB::fetch_all('SELECT COUNT(*) AS num, authorid FROM %t WHERE fid=%d GROUP BY authorid', array($this->get_table_name(), $fid));
	}
	public function count_group_by_fid($tableid = 0) {
		return Dxyz_DB::fetch_all('SELECT fid, COUNT(*) AS threads, SUM(replies)+COUNT(*) AS posts FROM %t GROUP BY fid', array($this->get_table_name($tableid)));
	}

	public function count_special_group_by_special() {
		return Dxyz_DB::fetch_all('SELECT special, count(*) AS spcount FROM %t GROUP BY special', array($this->get_table_name()));
	}
	...
	public function count_by_recyclebine($fid = 0, $isgroup = 0, $author = array(), $username = array(), $pstarttime = 0, $pendtime = 0, $mstarttime = 0, $mendtime = 0, $keywords = '') {
		$sql = $this->recyclebine_where($fid, $isgroup, $author, $username, $pstarttime, $pendtime, $mstarttime, $mendtime, $keywords);
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM '.Dxyz_DB::table('forum_thread').' t LEFT JOIN '.Dxyz_DB::table('forum_threadmod').' tm ON tm.tid=t.tid '.$sql[0], $sql[1]);
	}
	...
	public function fetch_all_for_guide($type, $limittid, $tids = array(), $heatslimit = 3, $dateline = 0) {
		...
		if(getglobal('setting/followforumid')) {
			$addsql .= ' AND '.Dxyz_DB::field('fid', getglobal('setting/followforumid'), '<>');
		}
		$tidsql = '';
		if($tids) {
			$tids = dintval($tids, true);
	...
	public function fetch_max_tid() {
		return Dxyz_DB::result_first("SELECT MAX(tid) as maxtid FROM ".Dxyz_DB::table('forum_thread'));
	}
	public function move_thread_by_tid($tids, $source, $target) {
		$source = intval($source);
		$target = intval($target);
		if($source != $target) {
			Dxyz_DB::query('REPLACE INTO %t SELECT * FROM %t WHERE tid IN (%n)', array($this->get_table_name($target), $this->get_table_name($source), $tids));
			return Dxyz_DB::delete($this->get_table_name($source), Dxyz_DB::field('tid', $tids));
		} else {
			return false;
		}
	}
	...
	function gettablestatus($tableid = 0) {
		$table_info = Dxyz_DB::fetch_first("SHOW TABLE STATUS LIKE '".str_replace('_', '\_', Dxyz_DB::table($this->get_table_name($tableid)))."'");
	...
	public function drop_table($tableid) {
		$tableid = intval($tableid);
		if($tableid) {
			Dxyz_DB::query("DROP TABLE %t", array($this->get_table_name($tableid)), true);
			return true;
		} else {
			return false;
		}
