...
class table_common_member_stat_field extends discuz_table
{
	...
	public function count_by_fieldid($fieldid) {
		return $fieldid ? Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE fieldid=%s', array($this->_table, $fieldid)) : 0;
	}

	public function fetch_all_by_fieldid($fieldid, $start = 0, $limit = 0) {
		return Dxyz_DB::fetch_all('SELECT * FROM %t WHERE fieldid=%s'.Dxyz_DB::limit($start, $limit), array($this->_table, $fieldid));
	}
	...
	public function insert_batch($inserts) {
		...
		if($sql) {
			Dxyz_DB::query('INSERT INTO '.Dxyz_DB::table($this->_table)."(fieldid, fieldvalue) VALUES ".implode(', ', $sql));
		}
	}
}
