...
class table_portal_topic_pic extends discuz_table
{
	...
	public function count_by_topicid($topicid) {
		return $topicid ? Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE topicid=%d', array($this->_table, $topicid)) : 0;
	}

	public function fetch_all_by_topicid($topicid, $start = 0, $limit = 0) {
		return $topicid ? Dxyz_DB::fetch_all('SELECT * FROM %t WHERE topicid=%d ORDER BY picid DESC'.Dxyz_DB::limit($start, $limit), array($this->_table, $topicid)) : array();
	}
