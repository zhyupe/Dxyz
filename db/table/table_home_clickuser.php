...
class table_home_clickuser extends discuz_table
{
	...
	public function delete_by_dateline($dateline) {
		return Dxyz_DB::query('DELETE FROM %t WHERE dateline<%d', array($this->_table, $dateline));
	}
	...
	public function count_by_uid_id_idtype($uid, $id, $idtype) {
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE uid=%d AND id=%d AND idtype=%s', array($this->_table, $uid, $id, $idtype));
	}
