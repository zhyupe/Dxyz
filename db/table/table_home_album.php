...
class table_home_album extends discuz_table
{
	...
	public function update_num_by_albumid($albumid, $inc, $field = 'picnum', $uid = '') {
		...
		return Dxyz_DB::query('UPDATE %t SET '.$field.'='.$field.'+\'%d\' WHERE albumid=%d '.$uidsql, $parameter);
	}
	...
	public function delete_by_uid($uid) {
		...
		return Dxyz_DB::delete($this->_table, Dxyz_DB::field('uid', $uid));
	}
	...
	public function fetch_albumid_by_albumname_uid($albumname, $uid) {
		return Dxyz_DB::result_first('SELECT albumid FROM %t WHERE albumname=%s AND uid=%d', array($this->_table, $albumname, $uid));
	}

	public function fetch_albumid_by_searchkey($searchkey, $limit) {
		return Dxyz_DB::fetch_all('SELECT albumid FROM %t WHERE 1 %i ORDER BY albumid DESC %i', array($this->_table, $searchkey, Dxyz_DB::limit(0, $limit)));
	}
	...
	public function fetch_all_by_uid($uid, $order = false, $start = 0, $limit = 0, $albumid = '') {
		...
		$wherearr = array();
		if($albumid) {
			$wherearr[] = Dxyz_DB::field('albumid', $albumid);
		}
		if($uid) {
			$wherearr[] = Dxyz_DB::field('uid', $uid);
		}
		if(is_string($order) && $order = Dxyz_DB::order($order, 'DESC')) {
		...
		return Dxyz_DB::fetch_all('SELECT * FROM %t '.$wheresql.$ordersql, $parameter);
	}

	public function fetch_all_by_block($aids, $bannedids, $uids, $catid, $startrow, $items, $orderby) {
		$wheres = array();
		if($aids) {
			$wheres[] = Dxyz_DB::field('albumid', $aids, 'in');
		}
		if($bannedids) {
			$wheres[]  = Dxyz_DB::field('albumid', $bannedids, 'notin');
		}
		if($uids) {
			$wheres[] = Dxyz_DB::field('uid', $uids, 'in');
		}
		if($catid && !in_array('0', $catid)) {
			$wheres[] = Dxyz_DB::field('catid', $catid, 'in');
		}
		$wheres[] = "friend = '0'";
		...
		return Dxyz_DB::fetch_all('SELECT * FROM '.Dxyz_DB::table($this->_table).' WHERE '.$wheresql.' ORDER BY '.Dxyz_DB::order($orderby, 'DESC').Dxyz_DB::limit($startrow, $items));
	}
	...
	public function fetch_all_by_search($fetchtype, $uids, $albumname, $searchname, $catid, $starttime, $endtime, $albumids, $friend = '', $orderfield = '', $ordersort = 'DESC', $start = 0, $limit = 0, $findex = '') {
		...
		if($fetchtype == 3) {
			...
			$selectfield = "*";
			if(is_string($orderfield) && $order = Dxyz_DB::order($orderfield, $ordersort)) {
				$ordersql = 'ORDER BY '.$order;
			}
			if($limit) {
				$parameter[] = Dxyz_DB::limit($start, $limit);
