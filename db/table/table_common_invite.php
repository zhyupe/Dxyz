...
class table_common_invite extends discuz_table
{
	...
	public function fetch_by_id_uid($id, $uid) {
		return Dxyz_DB::fetch_first('SELECT * FROM %t WHERE id=%d AND uid=%d', array($this->_table, $id, $uid));
	}
	public function fetch_by_code($code) {
		return Dxyz_DB::fetch_first('SELECT * FROM %t WHERE code=%s', array($this->_table, $code));
	}
	public function fetch_all_by_uid($uid) {
		return Dxyz_DB::fetch_all('SELECT * FROM %t WHERE uid=%d ORDER BY dateline DESC', array($this->_table, $uid), $this->_pk);
	}
	public function fetch_all_invitenum_group_by_uid($dateline = 0, $start = 0, $limit = 0) {
		...
		return Dxyz_DB::fetch_all("SELECT count(*) AS invitenum ,uid FROM %t WHERE status=2 $sql GROUP BY uid ORDER BY invitenum DESC ".Dxyz_DB::limit($start, $limit), $parameter);
	}
	public function fetch_all_orderid($orderid) {
		return Dxyz_DB::fetch_all('SELECT * FROM %t WHERE orderid=%s', array($this->_table, $orderid));
	}
	...
	public function count_by_uid_fuid($uid, $fuid) {
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE uid=%d AND fuid=%d', array($this->_table, $uid, $fuid));
	}
	public function count_by_uid_dateline($uid, $dateline) {
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE uid=%d AND dateline>%d', array($this->_table, $uid, $dateline));
	}
	...
	public function delete_by_uid_or_fuid($uids) {
		...
		Dxyz_DB::delete($this->_table, Dxyz_DB::field('uid', $uids).' OR '.Dxyz_DB::field('fuid', $uids));
	}
