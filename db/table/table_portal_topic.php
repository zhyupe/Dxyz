...
class table_portal_topic extends discuz_table
{
	...
	public function count_by_search_where($wherearr) {
		...
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM '.Dxyz_DB::table($this->_table).($wheresql ? ' WHERE '.$wheresql : ''));
	}

	public function fetch_all_by_search_where($wherearr, $ordersql, $start, $limit) {
		$wheresql = empty($wherearr) ? '' : implode(' AND ', $wherearr);
		return Dxyz_DB::fetch_all('SELECT * FROM '.Dxyz_DB::table($this->_table).($wheresql ? ' WHERE '.$wheresql : '').' '.$ordersql.Dxyz_DB::limit($start, $limit), null, 'topicid');
	}

	public function fetch_by_name($name) {
		return $name ? Dxyz_DB::fetch_first('SELECT * FROM %t WHERE name=%s LIMIT 1', array($this->_table, $name)) : false;
	}
