...
class table_common_credit_rule_log_field extends discuz_table
{
	...
	public function delete_clid($val) {
		Dxyz_DB::delete($this->_table, Dxyz_DB::field('clid', $val));
	}
	...
	public function update($uid, $clid, $data) {
		if(!empty($data) && is_array($data)) {
			return Dxyz_DB::update($this->_table, $data, array('uid'=>$uid, 'clid'=>$clid));
		}
		return 0;
	}
	...
	public function fetch($uid, $clid) {
		$logarr = array();
		if($uid && $clid) {
			$logarr = Dxyz_DB::fetch_first('SELECT * FROM %t WHERE uid=%d AND clid=%d', array($this->_table, $uid, $clid));
		}
