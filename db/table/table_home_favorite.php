...
class table_home_favorite extends discuz_table
{
	...
	public function fetch_by_id_idtype($id, $idtype, $uid = 0) {
		if($uid) {
			$uidsql = ' AND '.Dxyz_DB::field('uid', $uid);
		}
		return Dxyz_DB::fetch_first("SELECT * FROM %t WHERE id=%d AND idtype=%s $uidsql", array($this->_table, $id, $idtype));
	}
	...
	public function delete_by_id_idtype($id, $idtype) {
		return Dxyz_DB::delete($this->_table, Dxyz_DB::field('id', $id) .' AND '.Dxyz_DB::field('idtype', $idtype));
	}
	...
	public function delete($val, $unbuffered = false, $uid = 0) {
		...
		if($val) {
			...
			return Dxyz_DB::delete($this->_table, Dxyz_DB::field($this->_pk, $val).($uid ? ' AND '.Dxyz_DB::field('uid', $uid) : ''), null, $unbuffered);
		}
