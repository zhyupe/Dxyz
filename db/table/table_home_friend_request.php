...
class table_home_friend_request extends discuz_table
{
	...
	public function fetch_by_uid($uid) {
		return Dxyz_DB::fetch_first('SELECT * FROM %t WHERE uid=%d LIMIT 0,1', array($this->_table, $uid));
	}
	public function fetch_by_uid_fuid($uid, $fuid) {
		return Dxyz_DB::fetch_first('SELECT * FROM %t WHERE uid=%d AND fuid=%d', array($this->_table, $uid, $fuid));
	}
	...
	public function delete_by_uid_fuid($uid, $fuid) {
		...
		if($uid) {
			return Dxyz_DB::delete($this->_table, Dxyz_DB::field('uid', $uid).' AND '.Dxyz_DB::field('fuid', $fuid));
		}
		return 0;
	}
