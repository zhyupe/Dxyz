...
class table_common_domain extends discuz_table
{
	...
	public function update_by_idtype($idtype, $data) {
		if($idtype && !empty($data) && is_array($data)) {
			return Dxyz_DB::update($this->_table, $data, Dxyz_DB::field('idtype', $idtype));
		}
		return 0;
	}
	...
	public function fetch_all_by_idtype($idtype) {
		if(!empty($idtype)) {
			return Dxyz_DB::fetch_all('SELECT * FROM %t WHERE '.Dxyz_DB::field('idtype', $idtype), array($this->_table));
		}
		return array();
	}
	public function fetch_by_domain_domainroot($domain, $droot) {
		return Dxyz_DB::fetch_first('SELECT * FROM %t WHERE domain=%s AND domainroot=%s', array($this->_table, $domain, $droot));
	}
	...
	public function count_by_domain_domainroot($domain, $droot) {
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE domain=%s AND domainroot=%s', array($this->_table, $domain, $droot));
	}
