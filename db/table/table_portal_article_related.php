...
class table_portal_article_related extends discuz_table
{
	...
	public function delete_by_aid_raid($aid, $raid = null) {
		return ($aid = dintval($aid, true)) ? Dxyz_DB::delete($this->_table, Dxyz_DB::field('aid', $aid).($raid = dintval($raid) ? ' OR '.Dxyz_DB::field('raid', $raid) : '')) : false;
	}
	...
	public function insert_batch($aid, $list) {
		...
		if($replaces) {
			Dxyz_DB::query('REPLACE INTO '.Dxyz_DB::table($this->_table).' (aid,raid,displayorder) VALUES '.implode(',', $replaces));
		}
	}

	public function fetch_all_by_aid($aid) {
		return ($aid = dintval($aid)) ? Dxyz_DB::fetch_all('SELECT * FROM %t WHERE aid=%d ORDER BY displayorder', array($this->_table, $aid), 'raid') : array();
	}
}
