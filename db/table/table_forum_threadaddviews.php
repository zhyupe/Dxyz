...
class table_forum_threadaddviews extends discuz_table {
	...
	public function update_by_tid($tid) {
		return Dxyz_DB::query('UPDATE %t SET `addviews`=`addviews`+1 WHERE tid=%d', array($this->_table, $tid));
	}
	public function fetch_all_order_by_tid($start = 0, $limit = 0) {
		return Dxyz_DB::fetch_all('SELECT * FROM %t ORDER BY tid'.Dxyz_DB::limit($start, $limit), array($this->_table), $this->_pk);
	}
}
