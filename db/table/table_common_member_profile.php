...
class table_common_member_profile extends discuz_table_archive
{
	...
	public function fetch_all($uids, $force_from_Dxyz_DB = false, $fetch_archive = 1) {
		...
		if(!empty($uids)) {
			if($force_from_Dxyz_DB || ($data = $this->fetch_cache($uids)) === false || count($uids) != count($data)) {
				...
				if(!empty($uids)) {
					$query = Dxyz_DB::query('SELECT '.implode(',', $this->_fields).' FROM '.Dxyz_DB::table($this->_table).' WHERE '.Dxyz_DB::field($this->_pk, $uids));
	...
	public function count_by_field($field, $val) {
		...
		if(in_array($field, $this->_fields, true)) {
			$count = Dxyz_DB::result_first('SELECT COUNT(*) as cnt FROM '.Dxyz_DB::table('common_member_profile').' WHERE '.Dxyz_DB::field($field, $val));
		}
		return $count;
	}

	public function fetch_all_field_value($field) {
		return in_array($field, $this->_fields, true) ? Dxyz_DB::fetch_all('SELECT DISTINCT(`'.$field.'`) FROM '.Dxyz_DB::table($this->_table), null, $field) : array();
	}
	...
	public function fetch_all_will_birthday_by_uid($uids) {
		...
		if(!empty($uids)) {
			...
			$query = Dxyz_DB::query("SELECT sf.uid,sf.birthyear,sf.birthmonth,sf.birthday,s.username
				FROM ".Dxyz_DB::table('common_member_profile')." sf
				LEFT JOIN ".Dxyz_DB::table('common_member')." s USING(uid)
