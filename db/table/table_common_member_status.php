...
class table_common_member_status extends discuz_table_archive
{
	...
	public function count_by_ip($ips) {
		return !empty($ips) ? Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE regip IN(%n) OR lastip IN (%n)', array($this->_table, $ips, $ips)) : 0;
	}

	public function fetch_all_by_ip($ips, $start, $limit) {
		$data = array();
		if(!empty($ips) && $limit) {
			$data = Dxyz_DB::fetch_all('SELECT * FROM %t WHERE regip IN(%n) OR lastip IN (%n) LIMIT %d, %d', array($this->_table, $ips, $ips, $start, $limit), 'uid');
		}
		return $data;
	}
	...
	public function fetch_all_orderby_lastpost($uids, $start, $limit) {
		...
		if($uids) {
			return Dxyz_DB::fetch_all('SELECT * FROM %t WHERE uid IN(%n) ORDER BY lastpost DESC '.Dxyz_DB::limit($start, $limit), array($this->_table, $uids), $this->_pk);
		}
		return array();
	}
	...
	public function count_by_lastactivity_invisible($timestamp, $invisible = 0) {
		...
		return $timestamp ? Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE lastactivity >= %d'.$addsql, array($this->_table, $timestamp)) : 0;
	}
	...
	public function fetch_all_by_lastactivity_invisible($timestamp, $invisible = 0, $start = 0, $limit = 0) {
		...
		if($timestamp) {
			...
			$data = Dxyz_DB::fetch_all('SELECT * FROM %t WHERE lastactivity >= %d'.$addsql.' ORDER BY lastactivity DESC'.Dxyz_DB::limit($start, $limit), array($this->_table, $timestamp), $this->_pk);
		}
		return $data;
	}
