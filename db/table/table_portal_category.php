...
class table_portal_category extends discuz_table
{
	...
	public function increase($catids, $data) {
		...
		if(!empty($sql)){
			Dxyz_DB::query("UPDATE ".Dxyz_DB::table($this->_table)." SET ".implode(',', $sql)." WHERE catid IN (".dimplode($catids).")", 'UNBUFFERED');
		}
	}

	public function range($start = 0, $limit = 0) {
		$data = array();
		$query = Dxyz_DB::query('SELECT * FROM '.Dxyz_DB::table($this->_table).' ORDER BY displayorder,catid'.Dxyz_DB::limit($start, $limit));
