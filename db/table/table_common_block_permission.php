...
class table_common_block_permission extends discuz_table
{
	...
	public function fetch($bid, $uid){
		return ($bid = dintval($bid)) && ($uid = dintval($uid)) ? Dxyz_DB::fetch_first('SELECT * FROM %t WHERE bid=%d AND uid=%d', array($this->_table, $bid, $uid)) : array();
	}

	public function fetch_all_by_bid($bid, $uid = 0) {
		return ($bid = dintval($bid, true)) ? Dxyz_DB::fetch_all('SELECT * FROM %t WHERE bid=%d'.($uid ? ' AND '.Dxyz_DB::field('uid', $uid) : '').' ORDER BY inheritedtplname', array($this->_table, $bid), 'uid') : array();
	}
	...
	public function fetch_all_by_uid($uids, $flag = true, $sort = 'ASC', $start = 0, $limit = 0) {
		...
		return Dxyz_DB::fetch_all('SELECT * FROM '.Dxyz_DB::table($this->_table).$where.' ORDER BY uid '.$sort.', inheritedtplname'.Dxyz_DB::limit($start, $limit), NULL, ($uids && !is_array($uids)) ? 'bid' : '');
	}
	...
	public function fetch_permission_by_uid($uids) {
		return ($uids = dintval($uids, true)) ? Dxyz_DB::fetch_all('SELECT uid, sum(allowmanage) as allowmanage, sum(allowrecommend) as allowrecommend, sum(needverify) as needverify FROM '.Dxyz_DB::table($this->_table)." WHERE ".Dxyz_DB::field('uid', $uids)." GROUP BY uid", null, 'uid') : array();
	}

	public function delete_by_bid_uid_inheritedtplname($bid = false, $uids = false, $inheritedtplname = false) {
		$wherearr = array();
		if(($bid = dintval($bid, true))) {
			$wherearr[] = Dxyz_DB::field('bid', $bid);
		}
		if(($uids = dintval($uids, true))) {
			$wherearr[] = Dxyz_DB::field('uid', $uids);
		}
	...
	public function insert_batch($users, $bids, $tplname = '') {
		...
		if(!empty($users) && $bids = dintval($bids, true)){
			...
			if($blockperms) {
				Dxyz_DB::query('REPLACE INTO '.Dxyz_DB::table($this->_table).' (bid,uid,allowmanage,allowrecommend,needverify,inheritedtplname) VALUES '.implode(',', $blockperms));
				return $uids;
			} else {
				return FALSE;
			}
	...
	public function insert_by_bid($bid, $users) {
		...
		if(!empty($bid) && !empty($users)) {
			...
			if(!empty($sqlarr)) {
				Dxyz_DB::query('REPLACE INTO '.Dxyz_DB::table($this->_table).' (bid,uid,allowmanage,allowrecommend,needverify,inheritedtplname) VALUES '.implode(',', $sqlarr));
			}
		}
		return $uids;
	}
