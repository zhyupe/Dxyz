...
class table_common_stat extends discuz_table
{
	...
	public function updatestat($uid, $type, $primary = 0, $num = 1) {
		...
		$num = abs(intval($num));
		if(Dxyz_DB::result_first('SELECT COUNT(*) FROM '.Dxyz_DB::table($this->_table)." WHERE `daytime` = '$nowdaytime'")){
			Dxyz_DB::query('UPDATE '.Dxyz_DB::table($this->_table)." SET `$type`=`$type`+$num WHERE `daytime` = '$nowdaytime'");
		} else {
			C::t('common_statuser')->clear_by_daytime($nowdaytime);
			Dxyz_DB::insert($this->_table, array('daytime'=>$nowdaytime, $type=>$num));
		}
	}

	public function fetch_post_avg() {
		return Dxyz_DB::result_first("SELECT AVG(post) FROM ".Dxyz_DB::table($this->_table));
	}

	public function fetch_all($begin, $end, $field = '*') {
		$data = array();
		$query = Dxyz_DB::query('SELECT %i FROM %t WHERE daytime>=%d AND daytime<=%d ORDER BY daytime', array($field, $this->_table, $begin, $end));
	...
	public function fetch_all_by_daytime($daytime, $start = 0, $limit = 0, $sort = 'ASC') {
		...
		return Dxyz_DB::fetch_all("SELECT * FROM %t $wheresql ORDER BY daytime $sort".Dxyz_DB::limit($start, $limit), $parameter);
	}
}
