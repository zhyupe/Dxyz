...
class table_forum_threadcalendar extends discuz_table {
	...
	public function fetch_by_fid_dateline($fid, $dateline = 0, $order = 'dateline', $sort = 'DESC') {
		...
		return Dxyz_DB::fetch_first('SELECT * FROM %t '.$wheresql.' ORDER BY '.Dxyz_DB::order($order, $sort), $parameter, $this->_pk);
	}

	public function fetch_all_by_dateline($dateline) {
		$dateline = dintval($dateline);
		if($dateline) {
			return Dxyz_DB::fetch_all('SELECT * FROM %t WHERE dateline=%d', array($this->_table, $dateline), 'fid');
		} else {
			return array();
		}
	}
	...
	public function fetch_all_by_fid_dateline($fids, $dateline = 0) {
		...
		return Dxyz_DB::fetch_all('SELECT * FROM %t '.$wheresql, $parameter, 'fid');
	}
	...
	public function insert_multiterm($dataarr) {
		...
		if($sql) {
			return Dxyz_DB::query('INSERT INTO '.Dxyz_DB::table($this->_table)." (`fid`, `dateline`, `hotnum`) VALUES ".implode(',', $sql), true);
		}
		return false;
	}
}
