...
class table_common_member_verify extends discuz_table
{
	...
	public function fetch_all_by_vid($vid, $flag, $uids = array()) {
		...
		if($vid > 0 && $vid < 8) {
			...
			return Dxyz_DB::fetch_all("SELECT * FROM %t WHERE ".implode(' AND ', $wherearr), $parameter, $this->_pk);
		} else {
			return array();
		}
	}
	public function fetch_all_search($uid, $vid, $username = '', $order = 'dateline', $start = 0, $limit = 0, $sort = 'DESC') {
		...
		return Dxyz_DB::fetch_all("SELECT * FROM %t v, %t m  $condition[0] $ordersql ".Dxyz_DB::limit($start, $limit), $condition[1], $this->_pk);
	...
	public function count_by_search($uid, $vid, $username = '') {
		$condition = $this->search_condition($uid, $vid, $username);
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t v, %t m '.$condition[0], $condition[1]);
	}
