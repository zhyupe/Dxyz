...
class table_forum_collection extends discuz_table
{
	...
	public function fetch_all_by_uid($uid, $start = 0, $limit = 0, $exceptctid = null) {
		...
		return Dxyz_DB::fetch_all('SELECT * FROM %t WHERE uid=%d %i'.Dxyz_DB::limit($start, $limit), array($this->_table, $uid, $sql), $this->_pk);
	}

	public function range($start = 0, $limit = 0, $reqthread = 0, $pK = true) {
		return Dxyz_DB::fetch_all('SELECT * FROM %t WHERE threadnum>=%d ORDER BY lastupdate DESC '.Dxyz_DB::limit($start, $limit), array($this->_table, $reqthread), $pK ? $this->_pk : '');
	}
	...
	public function fetch($id, $force_from_Dxyz_DB = true){
