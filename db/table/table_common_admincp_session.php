...
class table_common_admincp_session extends discuz_table
{
	...
	public function fetch($uid, $panel) {
		...
		return Dxyz_DB::fetch_first($sql, array($this->_table, $uid, $panel));
	}

	public function fetch_all_by_panel($panel) {
		return Dxyz_DB::fetch_all('SELECT * FROM %t WHERE panel=%d', array($this->_table, $panel), 'uid');
	}
	...
	public function delete($uid, $panel, $ttl = 3600) {
		...
		Dxyz_DB::query($sql, array($this->_table, $uid, $panel, TIMESTAMP-intval($ttl)));

	}

	public function update($uid, $panel, $data) {
		if(!empty($data) && is_array($data)) {
			Dxyz_DB::update($this->_table, $data, array('uid'=>$uid, 'panel'=>$panel));
		}
	}
