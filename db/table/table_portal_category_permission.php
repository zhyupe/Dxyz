...
class table_portal_category_permission extends discuz_table
{
	...
	public function fetch($catid, $uid){
		return ($catid = dintval($catid)) && ($uid = dintval($uid)) ? Dxyz_DB::fetch_first('SELECT * FROM %t WHERE catid=%d AND uid=%d', array($this->_table, $catid, $uid)) : array();
	}

	public function fetch_all_by_catid($catid, $uid = 0) {
		return ($catid = dintval($catid)) ? Dxyz_DB::fetch_all('SELECT * FROM %t WHERE catid=%d'.($uid ? ' AND '.Dxyz_DB::field('uid', $uid) : ''), array($this->_table, $catid), 'uid') :array();
	}
	...
	public function fetch_all_by_uid($uids, $flag = true, $sort = 'ASC', $start = 0, $limit = 0) {
		...
		return Dxyz_DB::fetch_all('SELECT * FROM '.Dxyz_DB::table($this->_table).$where.' ORDER BY uid '.$sort.', inheritedcatid'.Dxyz_DB::limit($start, $limit), NULL, 'catid');
	}
	...
	public function fetch_permission_by_uid($uids) {
		return ($uids = dintval($uids, true)) ? Dxyz_DB::fetch_all('SELECT uid, sum(allowpublish) as allowpublish, sum(allowmanage) as allowmanage FROM '.Dxyz_DB::table($this->_table)." WHERE uid IN (".dimplode($uids).") GROUP BY uid", null, 'uid') : array();
	}
	...
	public function delete_by_catid_uid_inheritedcatid($catid = false, $uids = false, $inheritedcatid = false) {
		...
		if(($catid = dintval($catid, true))) {
			$wherearr[] = Dxyz_DB::field('catid', $catid);
		}
		if(($uids = dintval($uids, true))) {
			$wherearr[] = Dxyz_DB::field('uid', $uids);
		}
		if($inheritedcatid === true) {
			...
			$wherearr[] = Dxyz_DB::field('inheritedcatid', $inheritedcatid);
		}
	...
	public function insert_batch($users, $catids, $upid = 0) {
		...
		if(!empty($users) && ($catids = dintval((array)$catids, true))) {
			...
			if($perms) {
				Dxyz_DB::query('REPLACE INTO '.Dxyz_DB::table($this->_table).' (catid,uid,allowpublish,allowmanage,inheritedcatid) VALUES '.implode(',', $perms));
			}
		}
	}
}
