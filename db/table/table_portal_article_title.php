...
class table_portal_article_title extends discuz_table
{
	...
	public function fetch_all_aid_by_dateline($dateline, $catids = array(), $startid = 0, $endid = 0) {
		...
		$where = array();
		if($startid) {
			$where[] = Dxyz_DB::field('aid', intval($startid), '>=');
		}
		if($endid) {
			$where[] = Dxyz_DB::field('aid', intval($endid), '<=');
		}
		if($catids) {
			$where[] = Dxyz_DB::field('catid', dintval($catids, true));
		}
		if($dateline) {
			$where[] = Dxyz_DB::field('dateline', intval($dateline), '>=');
		}
		if($where) {
			$data = Dxyz_DB::fetch_all('SELECT aid FROM '.Dxyz_DB::table($this->_table).' WHERE '. implode(' AND ', $where).' LIMIT 200000', NULL, $this->_pk);
		}
		return $data;
	}
	...
	public function fetch_preaid_by_catid_aid($catid, $aid) {
		...
		if(($catid = intval($catid)) && ($aid = intval($aid))) {
			$ret = Dxyz_DB::result_first('SELECT aid FROM %t WHERE catid=%d AND aid<%d ORDER BY aid DESC LIMIT 1', array($this->_table, $catid, $aid));
		}
		return $ret;
	}
	...
	public function fetch_nextaid_by_catid_aid($catid, $aid) {
		$ret = 0;
		if(($catid = intval($catid)) && ($aid = intval($aid))) {
			$ret = Dxyz_DB::result_first('SELECT aid FROM %t WHERE catid=%d AND aid>%d ORDER BY aid ASC LIMIT 1', array($this->_table, $catid, $aid));
		}
		return $ret;
	}
}
