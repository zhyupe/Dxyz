...
class table_common_usergroup_field extends discuz_table
{
	...
	public function fetch_readaccess_by_readaccess($readaccess) {
		return Dxyz_DB::fetch_all('SELECT groupid,readaccess FROM %t WHERE readaccess>%d ORDER BY readaccess', array($this->_table, $readaccess), $this->_pk);
	}
	...
	public function fetch_all_fields($gid, $fields) {
		...
		foreach($fields as &$field) {
			$field = Dxyz_DB::quote_field($field);
		}
		$fieldssql = implode(',', $fields);
		return Dxyz_DB::fetch_all('SELECT %i FROM %t %i', array($fieldssql, $this->_table, ($gid ? 'WHERE '.Dxyz_DB::field('groupid', $gid) : '')), $this->_pk);
	}
	...
	public function count_by_field($field, $val, $glue = '=') {
		...
		return Dxyz_DB::result_first('SELECT count(*) FROM %t WHERE %i', array($this->_table, Dxyz_DB::field($field, $val, $glue)));
	}
	...
	public function update_allowsearch() {
		return Dxyz_DB::query('UPDATE %t SET allowsearch = allowsearch | 2 WHERE groupid < 20 AND groupid NOT IN (5, 6)', array($this->_table));
	}
