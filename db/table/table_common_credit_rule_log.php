...
class table_common_credit_rule_log extends discuz_table
{
	...
	public function increase($clid, $logarr) {
		if($clid && !empty($logarr) && is_array($logarr)) {
			return Dxyz_DB::query('UPDATE %t SET %i WHERE clid=%d', array($this->_table, implode(',', $logarr), $clid));
		}
		return 0;
	}
	...
	public function fetch_ids_by_rid_fid($rid, $fid) {
		$lids = array();
		if($rid && $fid) {
			$query = Dxyz_DB::query('SELECT clid FROM %t WHERE rid=%d AND fid=%d', array($this->_table, $rid, $fid));
	...
	public function fetch_rule_log($rid, $uid, $fid = 0){
		...
		if($rid && $uid) {
			...
			$log = Dxyz_DB::fetch_first('SELECT * FROM %t WHERE uid=%d AND rid=%d'.$sql, $para);
		}
		return $log;
	}
	...
	public function fetch_all_by_uid($uid, $start = 0, $limit = 0) {
		...
		$query = Dxyz_DB::query("SELECT * FROM %t WHERE uid=%d ORDER BY dateline DESC ".Dxyz_DB::limit($start, $limit), array($this->_table, $uid));
		while($value = Dxyz_DB::fetch($query)) {
