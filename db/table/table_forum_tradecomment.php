...
class table_forum_tradecomment extends discuz_table
{
	...
	function fetch_all_by_rateeid($rateeid, $type, $dateline = 0) {
		...
		return Dxyz_DB::fetch_all("SELECT * FROM %t WHERE rateeid=%d AND type=%d %i", array($this->_table, $rateeid, $type, $dateline));
	}
	...
	function fetch_all_list($from, $uid, $dateline, $score, $start) {
		...
		return Dxyz_DB::fetch_all("SELECT tc.*, tl.subject, tl.price, tl.credit FROM %t tc LEFT JOIN %t tl USING(orderid) WHERE %i %i %i ORDER BY tc.dateline DESC LIMIT %d, 10",
	...
	function count_list($from, $uid, $dateline, $score) {
		...
		return Dxyz_DB::result_first("SELECT COUNT(*) FROM %t tc WHERE %i %i %i", array($this->_table, $sql, $dateline, $score));
	}

	function get_month_score($uid, $type, $rateeid) {
		$monthfirstday = mktime(0, 0, 0, date('m', TIMESTAMP), 1, date('Y', TIMESTAMP));
		return Dxyz_DB::result_first("SELECT COUNT(score) FROM %t WHERE raterid=%d AND type=%d AND dateline>=%d AND rateeid=%d", array($this->_table, $uid, $type, $monthfirstday, $rateeid));
	}
