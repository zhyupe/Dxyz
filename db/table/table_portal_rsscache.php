...
class table_portal_rsscache extends discuz_table
{
	...
	public function fetch_all_by_catid($catid, $limit = 20) {
		return $catid ? Dxyz_DB::fetch_all('SELECT * FROM '.Dxyz_DB::table($this->_table).' WHERE '.Dxyz_DB::field('catid', $catid).' ORDER BY dateline DESC LIMIT '.dintval($limit), null, 'aid') : array();
	}
}
