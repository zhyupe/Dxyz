...
class table_common_moderate extends discuz_table
{
	...
	private function query_data($type, $idtype, $status = 0, $dateline = 0) {
		...
		if($type) {
			return Dxyz_DB::result_first("SELECT COUNT(*) FROM %t WHERE %i `status`=%d $othersql ORDER BY dateline DESC", $parameter);
		} else {
			return Dxyz_DB::fetch_all("SELECT * FROM %t WHERE %i `status`=%d $othersql ORDER BY dateline DESC", $parameter, 'id');
		}
	}
	public function fetch_all_for_article($status, $catid = 0, $username = '', $dateline = 'all', $count = 0, $start = 0, $limit = 0) {
		...
		if($count) {
			...
			FROM ".Dxyz_DB::table('portal_article_moderate')." m
			LEFT JOIN ".Dxyz_DB::table('portal_article_title')." a ON a.aid=m.id
			WHERE m.status='$status' $sqlwhere");
		}
		return Dxyz_DB::fetch_all("SELECT a.aid, a.catid, a.uid, a.username, a.title, a.summary, a.dateline, cat.catname
			FROM ".Dxyz_DB::table('portal_article_moderate')." m
			LEFT JOIN ".Dxyz_DB::table('portal_article_title')." a ON a.aid=m.id
			LEFT JOIN ".Dxyz_DB::table('portal_category')." cat ON cat.catid=a.catid
			WHERE m.status='$status' $sqlwhere
	...
	public function fetch_all_for_portalcomment($idtype, $tablename, $status, $catid = 0, $username = '', $dateline = 'all', $count = 0, $keyword = '', $start = 0, $limit = 0) {
		...
		if($count) {
			return Dxyz_DB::result_first("SELECT COUNT(*)
			FROM ".Dxyz_DB::table($this->_get_table($idtype.'_cid'))." m
			LEFT JOIN ".Dxyz_DB::table('portal_comment')." c ON c.cid=m.id
			LEFT JOIN ".Dxyz_DB::table($tablename)." a ON a.$idtype=c.id
			WHERE m.".Dxyz_DB::field('idtype', $idtype.'_cid')." AND m.status='$status' $sqlwhere");
		}
		return Dxyz_DB::fetch_all("SELECT c.cid, c.uid, c.username, c.id, c.postip, c.dateline, c.message, a.title
			FROM ".Dxyz_DB::table($this->_get_table($idtype.'_cid'))." m
			LEFT JOIN ".Dxyz_DB::table('portal_comment')." c ON c.cid=m.id
			LEFT JOIN ".Dxyz_DB::table($tablename)." a ON a.$idtype=c.id
			WHERE m.".Dxyz_DB::field('idtype', $idtype.'_cid')." AND m.status='$status' $sqlwhere
	...
	public function count_group_idtype_by_status($status) {
		...
		foreach($this->_tables as $idtype => $table) {
			if($this->_is_comment_table($idtype)) {
				$return[] = array('idtype' => $idtype, 'count' => Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE idtype=%s AND status=%d', array($table, $idtype, $status)));
			} else {
				$return[] = array('idtype' => $idtype, 'count' => Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE status=%d', array($table, $status)));
			}
		}
		return $return;
	}
	...
	public function delete($id, $idtype, $unbuffered = false) {
		...
		$wheresql = array();
		$id && $wheresql[] = Dxyz_DB::field('id', $id);
		$this->_is_comment_table($idtype) && $wheresql[] = Dxyz_DB::field('idtype', $idtype);
		return Dxyz_DB::delete($table, implode(' AND ', $wheresql), 0, $unbuffered);
	}
	...
	public function insert($idtype, $data, $return_insert_id = false, $replace = false, $silent = false) {
		...
		return Dxyz_DB::insert($table, $data, $return_insert_id, $replace, $silent);
	}
	...
	public function update($id, $idtype, $data, $unbuffered = false, $low_priority = false) {
		...
		$id && $wheresql[] = Dxyz_DB::field('id', $id);
		$this->_is_comment_table($idtype) && $wheresql[] = Dxyz_DB::field('idtype', $idtype);
		return Dxyz_DB::update($table, $data, implode(' AND ', $wheresql), $unbuffered, $low_priority);
	}
	...
	public function count_by_idtype_status_fid($idtype, $status, $fids) {
		...
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t m INNER JOIN %t t ON m.id=t.%i AND t.'.Dxyz_DB::field('fid', $fids).' WHERE m.status=%d',
	...
	public function count_by_search_for_post($posttable, $status = null, $first = null, $fids = null, $author = null, $dateline = null, $subject = null, $isgroup = null) {
		...
		if($first !== null) {
			$wheresql[] = 'p.'.Dxyz_DB::field('first', $first);
		}
		if($fids) {
	...
	public function count_by_seach_for_thread($status = null, $fids = null) {
		...
		if($fids) {
			$wheresql[] = 't.'.Dxyz_DB::field('fid', $fids);
		}
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t m LEFT JOIN %t t ON t.tid=m.id WHERE %i',
	...
	public function fetch_all_by_search_for_thread($status = null, $fids = null, $start = 0, $limit = 0) {
		...
		return Dxyz_DB::fetch_all('SELECT t.tid, t.fid, t.posttableid, t.author, t.sortid, t.authorid, t.subject as tsubject, t.dateline, t.attachment
			FROM %t m
	...
	public function count_by_search_for_blog($status = null, $username = null, $dateline = null, $subject = null) {
		...
		if($username) {
			$wheresql[] = 'b.'.Dxyz_DB::field('username', $username);
		}
		if($dateline) {
			$wheresql[] = 'b.'.Dxyz_DB::field('dateline', $dateline, '>');
		}
		if($subject) {
			$wheresql[] = 'b.'.Dxyz_DB::field('subject', '%'.$subject.'%', 'like');
		}
		return Dxyz_DB::result_first('SELECT COUNT(*)
			FROM %t m
	...
	public function fetch_all_by_search_for_blog($status = null, $username = null, $dateline = null, $subject = null, $start = 0, $limit = 0) {
		...
		return Dxyz_DB::fetch_all('SELECT b.blogid, b.uid, b.username, b.classid, b.subject, b.dateline, bf.message, bf.postip, c.classname
			FROM %t m
			LEFT JOIN %t b ON b.blogid=m.id
			LEFT JOIN %t bf ON bf.blogid=b.blogid
			LEFT JOIN %t c ON b.classid=c.classid
	...
	public function count_by_search_for_commnet($idtype = null, $status = null, $author = null, $dateline = null, $message = null) {
		...
		if($idtype) {
			...
			$wheresql[] = 'm.'.Dxyz_DB::field('idtype', $idtype.'_cid');
		} else {
		...
		if($author) {
			$wheresql[] = 'c.'.Dxyz_DB::field('author', $author);
		}
		if($dateline) {
			$wheresql[] = 'c.'.Dxyz_DB::field('dateline', $dateline, '>');
		}
		if($message) {
			$message = str_replace(array('_', '%'), array('\_', '\%'), $message);
			$wheresql[] = 'c.'.Dxyz_DB::field('message', '%'.$message.'%', 'like');
		}
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t m LEFT JOIN %t c ON c.cid=m.id WHERE %i',
	...
	public function fetch_all_by_search_for_comment($idtype = null, $status = null, $author = null, $dateline = null, $message = null, $start = 0, $limit = 0) {
		...
		return Dxyz_DB::fetch_all('SELECT c.cid, c.uid, c.id, c.idtype, c.authorid, c.author, c.message, c.dateline, c.ip
			FROM %t m
			LEFT JOIN %t c ON c.cid=m.id
			WHERE %i
			ORDER BY c.dateline DESC '.Dxyz_DB::limit($start, $limit),
			array($table, 'home_comment', implode(' AND ', $wheresql)));
	}
	...
	public function count_by_search_for_doing($status = null, $username = null, $dateline = null, $message = null) {
		...
		if($username) {
			$wheresql[] = 'd.'.Dxyz_DB::field('username', $username);
		}
		if($dateline) {
			$wheresql[] = 'd.'.Dxyz_DB::field('dateline', $dateline, '>');
		}
		if($message) {
			$message = str_replace(array('_', '%'), array('\_', '\%'), $message);
			$wheresql[] = 'd.'.Dxyz_DB::field('message', '%'.$message.'%', 'like');
		}
		return Dxyz_DB::result_first('SELECT COUNT(*)
			FROM %t m
	...
	public function fetch_all_by_search_for_doing($status = null, $username = null, $dateline = null, $message = null, $start = 0, $limit = 0) {
		...
		return Dxyz_DB::fetch_all('SELECT d.doid, d.uid, d.username, d.dateline, d.message, d.ip
			FROM %t m
			LEFT JOIN %t d ON d.doid=m.id
			WHERE %i
			ORDER BY m.dateline DESC '.Dxyz_DB::limit($start, $limit),
	...
	public function count_by_search_for_pic($status = null, $username = null, $dateline = null, $title = null) {
		...
		if($username) {
			$wheresql[] = 'p.'.Dxyz_DB::field('username', $username);
		}
		if($dateline) {
			$wheresql[] = 'p.'.Dxyz_DB::field('dateline', $dateline, '>');
		}
		if($title) {
			$wheresql[] = 'p.'.Dxyz_DB::field('title', '%'.$title.'%', 'like');
		}
		return Dxyz_DB::result_first('SELECT COUNT(*)
			FROM %t m
	...
	public function fetch_all_by_search_for_pic($status = null, $username = null, $dateline = null, $title = null, $start = 0, $limit = 0) {
		...
		return Dxyz_DB::fetch_all('SELECT p.picid, p.albumid, p.uid, p.username, p.title, p.dateline, p.filepath, p.thumb, p.remote, p.postip, a.albumname
			FROM %t m
			LEFT JOIN %t p ON p.picid=m.id
	...
	public function delete_by_status_idtype($status, $idtype) {
		...
		$table = $this->_get_table($idtype);
		$idtype = $table == 'home_comment_moderate' ? Dxyz_DB::field('idtype', $idtype).' AND' : '';
		return Dxyz_DB::query('DELETE FROM %t WHERE %i status=%d', array($table, $idtype, $status));
	}
	...
	public function count_by_search_for_share($status = null, $username = null, $dateline = null, $body_general = null) {
		...
		if($username) {
			$wheresql[] = 's.'.Dxyz_DB::field('username', $username);
		}
		if($dateline) {
			$wheresql[] = 's.'.Dxyz_DB::field('dateline', $dateline, '>');
		}
		if($body_general) {
			$body_general = str_replace(array('%', '_'), array('\%', '\_'), $body_general);
			$wheresql[] = 's.'.Dxyz_DB::field('body_general', '%'.$body_general.'%', 'like');
		}
		return Dxyz_DB::result_first('SELECT COUNT(*)
			FROM %t m
	...
	public function fetch_all_by_search_for_share($status = null, $username = null, $dateline = null, $body_general = null, $start = 0, $limit = 0) {
		...
		return Dxyz_DB::fetch_all('SELECT s.sid, s.type, s.uid, s.username, s.dateline, s.body_general, s.itemid, s.fromuid
			FROM %t m
			LEFT JOIN %t s ON s.sid=m.id
			WHERE %i
			ORDER BY m.dateline DESC '.Dxyz_DB::limit($start, $limit),
