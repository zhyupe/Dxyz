...
class table_common_credit_rule extends discuz_table
{
	...
	public function fetch_all_by_rid($rid = 0) {
		...
		return Dxyz_DB::fetch_all("SELECT * FROM %t $wheresql ORDER BY rid DESC", $parameter, $this->_pk);
	}

	public function fetch_all_rule() {
		return Dxyz_DB::fetch_all('SELECT * FROM %t ORDER BY rid DESC', array($this->_table));
	}

	public function fetch_all_by_action($action) {
		if(!empty($action)) {
			return Dxyz_DB::fetch_all('SELECT * FROM %t WHERE action IN(%n)', array($this->_table, $action), $this->_pk);
		}
		return array();
	}
