...
class table_forum_post extends discuz_table
{
	...
	public function count_author_by_tid($tid) {
		return Dxyz_DB::result_first('SELECT count(DISTINCT authorid) FROM %t WHERE tid=%d', array(self::get_tablename('tid:'.$tid), $tid));
	}

	public function count_by_tid_dateline($tableid, $tid, $dateline) {
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE tid=%d AND invisible=0 AND dateline<=%d',
				array(self::get_tablename($tableid), $tid, $dateline));
	}

	public function fetch_maxposition_by_tid($tableid, $tid) {
		return Dxyz_DB::result_first('SELECT position FROM %t WHERE tid=%d ORDER BY position DESC LIMIT 1',
	...
	public function fetch_all_by_tid_range_position($tableid, $tid, $start, $end, $maxposition, $ordertype = 0) {
		...
		$data = Dxyz_DB::fetch_all('SELECT * FROM %t WHERE tid=%d AND position>=%d AND position<%d ORDER BY position'.($ordertype == 1 ? ' DESC' : ''), array(self::get_tablename($tableid), $tid, $start, $end), 'pid');
		if($storeflag) {
	...
	public function fetch_all_by_tid_position($tableid, $tid, $position) {
		return Dxyz_DB::fetch_all('SELECT * FROM %t WHERE tid=%d AND '.Dxyz_DB::field('position', $position), array(self::get_tablename($tableid), $tid));
	}
	public function count_by_tid_invisible_authorid($tid, $authorid) {
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE tid=%d AND invisible=0 AND authorid=%d',
				array(self::get_tablename('tid:'.$tid), $tid, $authorid));
	}

	public function count_visiblepost_by_tid($tid) {
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE tid=%d AND invisible=0', array(self::get_tablename('tid:'.$tid), $tid));
	}

	public function count_by_tid_pid($tid, $pid) {
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE tid=%d AND pid<%d', array(self::get_tablename('tid:'.$tid), $tid, $pid));
	}

	public function count_by_tid_authorid($tid, $authorid) {
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE tid=%d AND first=0 AND authorid=%d', array(self::get_tablename('tid:'.$tid), $tid, $authorid));
	}

	public function count_by_authorid($tableid, $authorid) {
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE authorid=%d AND invisible=0', array(self::get_tablename($tableid), $authorid));
	}

	public function count_group_authorid_by_fid($tableid, $fid) {
		return Dxyz_DB::fetch_all('SELECT COUNT(*) as num, authorid FROM %t WHERE fid=%d AND first=0 GROUP BY authorid', array(self::get_tablename($tableid), $fid));
	}

	public function count_by_first($tableid, $first) {
		return Dxyz_DB::result_first('SELECT count(*) FROM %t WHERE %i', array(self::get_tablename($tableid), Dxyz_DB::field('first', $first)));
	}

	public function count_by_invisible($tableid, $invisible) {
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE %i', array(self::get_tablename($tableid), Dxyz_DB::field('invisible', $invisible)));
	}

	public function count_table($tableid) {
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t', array(self::get_tablename($tableid)));
	}

	public function count_by_fid_invisible($tableid, $fid, $invisible) {
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE fid=%d AND invisible=%d', array(self::get_tablename($tableid), $fid, $invisible));
	}

	public function count_by_dateline($tableid, $dateline) {
		return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE dateline>=%d AND invisible=0', array(self::get_tablename($tableid), $dateline));
	}

	public function fetch($tableid, $pid, $outmsg = true) {
		$post = Dxyz_DB::fetch_first('SELECT * FROM %t WHERE pid=%d', array(self::get_tablename($tableid), $pid));
		if(!$outmsg) {
			unset($post['message']);
		}
		return $post;
	...
	public function fetch_visiblepost_by_tid($tableid, $tid, $start = 0, $order = 0) {
		return Dxyz_DB::fetch_first('SELECT * FROM %t WHERE tid=%d AND invisible=0 ORDER BY dateline '. ($order ? 'DESC' : '').' '. Dxyz_DB::limit($start, 1),
				array(self::get_tablename($tableid), $tid));
	}

	public function fetch_threadpost_by_tid_invisible($tid, $invisible = null) {
		return Dxyz_DB::fetch_first('SELECT * FROM %t WHERE tid=%d AND first=1'.($invisible !== null ? ' AND '.Dxyz_DB::field('invisible', $invisible) : ''),
				array(self::get_tablename('tid:'.$tid), $tid));
	}

	public function fetch_pid_by_tid_authorid($tid, $authorid) {
		return Dxyz_DB::result_first('SELECT pid FROM %t WHERE tid=%d AND authorid=%d LIMIT 1', array(self::get_tablename('tid:'.$tid), $tid, $authorid));
	}

	public function fetch_pid_by_tid_clientip($tid, $clientip) {
		return Dxyz_DB::result_first('SELECT pid FROM %t WHERE tid=%d AND authorid=0 AND useip=%s LIMIT 1', array(self::get_tablename('tid:'.$tid), $tid, $clientip));
	}

	public function fetch_attachment_by_tid($tid) {
		return Dxyz_DB::result_first('SELECT attachment FROM %t WHERE tid=%d AND invisible=0 AND attachment>0 LIMIT 1', array(self::get_tablename('tid:'.$tid), $tid));
	}

	public function fetch_maxid($tableid) {
		return Dxyz_DB::result_first('SELECT MAX(pid) FROM %t', array(self::get_tablename($tableid)));
	}

	public function fetch_posts($tableid) {
		return Dxyz_DB::fetch_first('SELECT COUNT(*) AS posts, (MAX(dateline)-MIN(dateline))/86400 AS runtime FROM %t', array(self::get_tablename($tableid)));
	}

	public function fetch_by_pid_condition($tableid, $pid, $addcondiction = '', $fields = '*') {
		return Dxyz_DB::fetch_first('SELECT %i FROM %t WHERE pid=%d %i LIMIT 1',
	...
	public function fetch_all($tableid, $pids, $outmsg = true) {
		$postlist = array();
		if($pids) {
			$query = Dxyz_DB::query('SELECT * FROM %t WHERE %i', array(self::get_tablename($tableid), Dxyz_DB::field($this->_pk, $pids)));
			while($post = Dxyz_DB::fetch($query)) {
				if(!$outmsg) {
					unset($post['message']);
				}
	...
	public function fetch_all_debatepost_viewthread_by_tid($tid, $visibleallflag, $authorid, $stand, $forum_pagebydesc, $ordertype, $start, $limit) {
		...
		$query = Dxyz_DB::query("SELECT dp.*, p.* FROM %t p LEFT JOIN %t dp ON p.pid=dp.pid WHERE p.tid=%d".
				($parameter['invisible'] ? ' AND '.$parameter['invisible'] : '').($parameter['authorid'] ? ' AND '.$parameter['authorid'] : '').
	...
	private function handle_viewthread_parameter($visibleallflag, $authorid, $forum_pagebydesc, $ordertype, $alias = '') {
		$return = array();
		if(!$visibleallflag) {
			$return['invisible'] = $alias.Dxyz_DB::field('invisible', 0);
		}
		if($authorid) {
			$return['authorid'] = $alias.Dxyz_DB::field('authorid', $authorid);
		}
		if($forum_pagebydesc) {
			if($ordertype != 1) {
	...
	public function fetch_all_by_authorid($tableid, $authorid, $outmsg = true, $order = '', $start = 0, $limit = 0, $first = null, $invisible = null, $fid = null, $filterfid = null) {
		...
		if($first !== null && $invisible !== null) {
			if($first == 1) {
				$sql[] = Dxyz_DB::field('invisible', $invisible);
				$sql[] = Dxyz_DB::field('first', 1);
			} else {
				$sql[] = Dxyz_DB::field('invisible', $invisible);
				$sql[] = Dxyz_DB::field('first', 0);
			}
		} elseif($invisible !== null) {
			$sql[] = Dxyz_DB::field('invisible', $invisible);
		} elseif($first !== null) {
			$sql[] = Dxyz_DB::field('first', $first);
		}
		if($fid !== null) {
			$sql[] = Dxyz_DB::field('fid', $fid);
		}
		if($filterfid !== null) {
			$filterfid = dintval($filterfid, true);
			$sql[] = Dxyz_DB::field('fid', $filterfid, is_array($filterfid) ? 'notin' : '<>');
		}
		$query = Dxyz_DB::query('SELECT * FROM %t WHERE '.Dxyz_DB::field('authorid', $authorid).' %i '.($order ? 'ORDER BY dateline '.$order : '').' '.Dxyz_DB::limit($start, $limit),
	...
	public function fetch_all_tid_by_first($tableid, $first, $start = 0, $limit = 0) {
		return Dxyz_DB::fetch_all('SELECT tid FROM %t WHERE first=%d '.Dxyz_DB::limit($start, $limit), array(self::get_tablename($tableid), $first));
	}
	...
	public function fetch_all_by_tid($tableid, $tids, $outmsg = true, $order = '', $start = 0, $limit = 0, $first = null, $invisible = null, $authorid = null, $fid = null) {
		...
		if($authorid !== null) {
			$sql[] = Dxyz_DB::field('authorid', $authorid);
		}
		if($fid !== null) {
			$sql[] = Dxyz_DB::field('fid', $fid);
		}
		$query = Dxyz_DB::query('SELECT * FROM %t WHERE '.Dxyz_DB::field('tid', $tids).' %i '.($order ? 'ORDER BY dateline '.$order : '').' '.Dxyz_DB::limit($start, $limit),
				array(self::get_tablename($tableid), ($sql ? 'AND '.implode(' AND ', $sql) : '')));
		while($post = Dxyz_DB::fetch($query)) {
			if(!$outmsg) {
				unset($post['message']);
	...
	public function fetch_all_pid_by_tid_lastpid($tid, $lastpid, $round) {
		return Dxyz_DB::fetch_all("SELECT pid FROM %t WHERE tid=%d AND pid>%d ORDER BY pid ASC %i",
				array(self::get_tablename('tid:'.$tid), $tid, $lastpid, Dxyz_DB::limit(0, $round)));
	}
	...
	public function fetch_all_by_fid($tableid, $fid, $outmsg = true, $order = '', $start = 0, $limit = 0, $first = null, $invisible = null) {
		...
		$query = Dxyz_DB::query('SELECT * FROM %t WHERE '.Dxyz_DB::field('fid', $fid).' %i '.($order ? 'ORDER BY dateline '.$order : '').' '.Dxyz_DB::limit($start, $limit),
				array(self::get_tablename($tableid), ($sql ? 'AND '.implode(' AND ', $sql) : '')));
		while($post = Dxyz_DB::fetch($query)) {
			if(!$outmsg) {
				unset($post['message']);
	...
	public function fetch_all_by_pid($tableid, $pids, $outmsg = true, $order = '', $start = 0, $limit = 0, $fid = null, $invisible = null) {
		...
		$query = Dxyz_DB::query('SELECT * FROM %t WHERE '.Dxyz_DB::field('pid', $pids).' %i '.($order ? 'ORDER BY dateline '.$order : '').' '.Dxyz_DB::limit($start, $limit),
				array(self::get_tablename($tableid), ($sql ? 'AND '.implode(' AND ', $sql) : '')));
		while($post = Dxyz_DB::fetch($query)) {
			if(!$outmsg) {
				unset($post['message']);
	...
	public function fetch_all_debatepost_by_tid_stand($tid, $stand, $start, $limit) {
		return Dxyz_DB::fetch_all('
			...
			array(self::get_tablename('tid:'.$tid), 'forum_debatepost', $tid, $stand, Dxyz_DB::limit($start, $limit)));
	}

	public function fetch_all_visiblepost_by_tid_groupby_authorid($tableid, $tid) {
		return Dxyz_DB::fetch_all('SELECT pid, tid, authorid, subject, dateline FROM %t WHERE tid=%d AND invisible=0 GROUP BY authorid ORDER BY dateline',
				array(self::get_tablename($tableid), $tid));
	}

	public function fetch_all_pid_by_invisible_dateline($tableid, $invisible, $dateline, $start, $limit) {
		return Dxyz_DB::fetch_all('SELECT pid FROM %t WHERE invisible=%d AND dateline<%d %i', array(self::get_tablename($tableid), $invisible, $dateline, Dxyz_DB::limit($start, $limit)));
	}

	public function fetch_all_top_post_author($tableid, $timelimit, $num) {
		return Dxyz_DB::fetch_all('SELECT DISTINCT(author) AS username, authorid AS uid, COUNT(pid) AS posts
			FROM %t
			WHERE dateline>=%d AND invisible=0 AND authorid>0
			GROUP BY author
			ORDER BY posts DESC %i',
			array(self::get_tablename($tableid), $timelimit, Dxyz_DB::limit(0, $num)));
	}

	public function fetch_all_author_posts_by_dateline($tableid, $authorid, $dateline) {
		return Dxyz_DB::fetch_all('SELECT authorid, COUNT(*) AS posts FROM %t
			WHERE dateline>=%d AND %i AND invisible=0 GROUP BY authorid', array(self::get_tablename($tableid), $dateline, Dxyz_DB::field('authorid', $authorid)));
	}
	...
	public function update($tableid, $pid, $data, $unbuffered = false, $low_priority = false, $first = null, $invisible = null, $fid = null, $status = null) {
		$where = array();
		$where[] = Dxyz_DB::field('pid', $pid);
		if($first !== null) {
			$where[] = Dxyz_DB::field('first', $first);
		}
		if($invisible !== null) {
			$where[] = Dxyz_DB::field('invisible', $invisible);
		}
		if($status !== null) {
			$where[] = Dxyz_DB::field('status', $status);
		}
		if($fid !== null) {
			$where[] = Dxyz_DB::field('fid', $fid);
		}
		$return = Dxyz_DB::update(self::get_tablename($tableid), $data, implode(' AND ', $where), $unbuffered, $low_priority);
	...
	public function update_fid_by_fid($tableid, $fid, $newfid, $unbuffered = false, $low_priority = false) {
		$where = array();
		$where[] = Dxyz_DB::field('fid', $fid);
		$return = Dxyz_DB::update(self::get_tablename($tableid), array('fid' => $newfid), implode(' AND ', $where), $unbuffered, $low_priority);
		if($return && $this->_allowmem) {
			$updatefid = $this->fetch_cache('updatefid');
	...
	public function update_cache($tableid, $id, $idtype, $data, $condition = array(), $glue = 'merge') {
		...
		if($idtype == 'tid') {
			...
			$memorytid = array();
			$query = Dxyz_DB::query('SELECT pid, tid FROM %t WHERE '.Dxyz_DB::field('pid', $id), array(self::get_tablename($tableid)));
			while($post = Dxyz_DB::fetch($query)) {
	...
	public function concat_threadtags_by_tid($tid, $tags) {
		$return = Dxyz_DB::query('UPDATE %t SET tags=concat(tags, %s) WHERE tid=%d AND first=1', array(self::get_tablename('tid:'.$tid), $tags, $tid));
		if($return && $this->_allowmem) {
	...
	public function increase_rate_by_pid($tableid, $pid, $rate, $ratetimes) {
		$return = Dxyz_DB::query('UPDATE %t SET rate=rate+\'%d\', ratetimes=ratetimes+\'%d\' WHERE pid=%d',
	...
	public function increase_position_by_tid($tableid, $tid, $position) {
		$return = Dxyz_DB::query('UPDATE %t SET position=position+\'%d\' WHERE '.Dxyz_DB::field('tid', $tid),
				array(self::get_tablename($tableid), $position));
		return $return;
	}

	public function increase_status_by_pid($tableid, $pid, $status, $glue, $unbuffered = false) {
		$return = Dxyz_DB::query('UPDATE %t SET %i WHERE %i', array(self::get_tablename($tableid), Dxyz_DB::field('status', $status, $glue), Dxyz_DB::field('pid', $pid)), $unbuffered);
		if($return && $this->_allowmem) {
	...
	public function insert($tableid, $data, $return_insert_id = false, $replace = false, $silent = false) {
		return Dxyz_DB::insert(self::get_tablename($tableid), $data, $return_insert_id, $replace, $silent);
	}

	public function delete($tableid, $pid, $unbuffered = false) {
		$return = Dxyz_DB::delete(self::get_tablename($tableid), Dxyz_DB::field($this->_pk, $pid), 0, $unbuffered);
		if($return && $this->_allowmem) {
			$delpid = $this->fetch_cache('delpid');
	...
	public function delete_by_tid($tableid, $tids, $unbuffered = false) {
		$return = Dxyz_DB::delete(self::get_tablename($tableid), Dxyz_DB::field('tid', $tids), 0, $unbuffered);
		if($return && $this->_allowmem) {
	...
	public function delete_by_authorid($tableid, $authorids, $unbuffered = false) {
		$return = Dxyz_DB::delete(self::get_tablename($tableid), Dxyz_DB::field('authorid', $authorids), 0, $unbuffered);
		if($return && $this->_allowmem) {
			$delauthorid = $this->fetch_cache('delauthorid');
	...
	public function delete_by_fid($tableid, $fids, $unbuffered = false) {
		return Dxyz_DB::delete(self::get_tablename($tableid), Dxyz_DB::field('fid', $fids), 0, $unbuffered);
	}

	public function show_table() {
		return Dxyz_DB::fetch_all("SHOW TABLES LIKE '".Dxyz_DB::table('forum_post')."\_%'");
	}

	public function show_table_by_tableid($tableid) {
		return Dxyz_DB::fetch_first('SHOW CREATE TABLE %t', array(self::get_tablename($tableid)));
	}

	public function drop_table($tableid) {
		return ($tableid = dintval($tableid)) ? Dxyz_DB::query('DROP TABLE %t', array(self::get_tablename($tableid))) : false;
	}

	public function optimize_table($tableid) {
		return Dxyz_DB::query('OPTIMIZE TABLE %t', array(self::get_tablename($tableid)), true);
	}

	public function move_table($tableid, $fieldstr, $fromtable, $tid) {
		$tidsql = is_array($tid) ? 'tid IN(%n)' : 'tid=%d';
		return Dxyz_DB::query("INSERT INTO %t ($fieldstr) SELECT $fieldstr FROM $fromtable WHERE $tidsql", array(self::get_tablename($tableid), $tid), true);
	}
	...
	public function count_by_search($tableid, $tid = null, $keywords = null, $invisible =null, $fid = null, $authorid = null, $author = null, $starttime = null, $endtime = null, $useip = null, $first = null) {
		$sql = '';
		$sql .= $tid ? ' AND '.Dxyz_DB::field('tid', $tid) : '';
		$sql .= $authorid !== null ? ' AND '.Dxyz_DB::field('authorid', $authorid) : '';
		$sql .= $invisible !== null ? ' AND '.Dxyz_DB::field('invisible', $invisible) : '';
		$sql .= $first !== null ? ' AND '.Dxyz_DB::field('first', $first) : '';
		$sql .= $fid ? ' AND '.Dxyz_DB::field('fid', $fid) : '';
		$sql .= $author ? ' AND '.Dxyz_DB::field('author', $author) : '';
		$sql .= $starttime ? ' AND '.Dxyz_DB::field('dateline', $starttime, '>=') : '';
		$sql .= $endtime ? ' AND '.Dxyz_DB::field('dateline', $endtime, '<') : '';
		$sql .= $useip ? ' AND '.Dxyz_DB::field('useip', $useip, 'like') : '';
		if(trim($keywords)) {
			$sqlkeywords = $or = '';
		...
		if($sql) {
			return Dxyz_DB::result_first('SELECT COUNT(*) FROM %t WHERE 1 %i', array(self::get_tablename($tableid), $sql));
		} else {
			return 0;
		}
	}
	...
	public function fetch_all_by_search($tableid, $tid = null, $keywords = null, $invisible = null, $fid = null, $authorid = null, $author = null, $starttime = null, $endtime = null, $useip = null, $first = null, $start = null, $limit = null) {
		$sql = '';
		$sql .= $tid ? ' AND '.Dxyz_DB::field('tid', $tid) : '';
		$sql .= $authorid ? ' AND '.Dxyz_DB::field('authorid', $authorid) : '';
		$sql .= $invisible !== null ? ' AND '.Dxyz_DB::field('invisible', $invisible) : '';
		$sql .= $first !== null ? ' AND '.Dxyz_DB::field('first', $first) : '';
		$sql .= $fid ? ' AND '.Dxyz_DB::field('fid', $fid) : '';
		$sql .= $author ? ' AND '.Dxyz_DB::field('author', $author) : '';
		...
		if($sql) {
			return Dxyz_DB::fetch_all('SELECT * FROM %t WHERE 1 %i ORDER BY dateline DESC %i', array(self::get_tablename($tableid), $sql, Dxyz_DB::limit($start, $limit)));
		} else {
			return array();
		}
	}
	...
	public function count_prune_by_search($tableid, $isgroup = null, $keywords = null, $message_length = null, $fid = null, $authorid = null, $starttime = null, $endtime = null, $useip = null) {
		$sql = '';
		$sql .= $fid ? ' AND p.'.Dxyz_DB::field('fid', $fid) : '';
		$sql .= $isgroup ? ' AND t.'.Dxyz_DB::field('isgroup', $isgroup) : '';
		$sql .= $authorid !== null ? ' AND p.'.Dxyz_DB::field('authorid', $authorid) : '';
		$sql .= $starttime ? ' AND p.'.Dxyz_DB::field('dateline', $starttime, '>=') : '';
		$sql .= $endtime ? ' AND p.'.Dxyz_DB::field('dateline', $endtime, '<') : '';
		$sql .= $useip ? ' AND p.'.Dxyz_DB::field('useip', $useip, 'like') : '';
		...
		if($sql) {
			if($isgroup) {
				return Dxyz_DB::result_first('SELECT COUNT(*)
	...
	public function fetch_all_new_post_by_pid($pid, $start = 0, $limit = 0, $tableid = 0, $glue = '>', $sort = 'ASC') {
		return $limit ? Dxyz_DB::fetch_all('SELECT * FROM '.Dxyz_DB::table($this->get_tablename($tableid)).
				' WHERE '.Dxyz_DB::field('pid', $pid, $glue).
				' ORDER BY '.Dxyz_DB::order('pid', $sort).
				Dxyz_DB::limit($start, $limit), $this->_pk) : array();
	}
	public function fetch_all_prune_by_search($tableid, $isgroup = null, $keywords = null, $message_length = null, $fid = null, $authorid = null, $starttime = null, $endtime = null, $useip = null, $outmsg = true, $start = null, $limit = null) {
		...
		if($sql) {
			if($isgroup) {
				$query = Dxyz_DB::query('SELECT p.*, t.*
					FROM %t p LEFT JOIN %t t USING(tid)
					WHERE 1 %i %i', array(self::get_tablename($tableid), 'forum_thread', $sql, Dxyz_DB::limit($start, $limit)));
			} else {
				$query = Dxyz_DB::query('SELECT *
					FROM %t p
					WHERE 1 %i %i', array(self::get_tablename($tableid), $sql, Dxyz_DB::limit($start, $limit)));
			}
			while($post = Dxyz_DB::fetch($query)) {
				if(!$outmsg) {
					unset($post['message']);
	...
	public static function getposttablebytid($tids, $primary = 0) {
		...
		foreach($threadtableids as $tableid) {
			...
			$query = Dxyz_DB::query("SELECT tid, posttableid FROM ".Dxyz_DB::table($threadtable)." WHERE tid IN(".dimplode(array_keys($tids)).")");
	...
	public static function getposttable($tableid = 0, $prefix = false) {
		...
		if($prefix) {
			$tablename = Dxyz_DB::table($tablename);
		}
		return $tablename;
	}
