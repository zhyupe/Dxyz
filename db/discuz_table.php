...
class discuz_table extends discuz_base
{
	...
	public function count() {
		$count = (int) Dxyz_DB::result_first("SELECT count(*) FROM ".Dxyz_DB::table($this->_table));
		return $count;
	}
	...
	public function update($val, $data, $unbuffered = false, $low_priority = false) {
		if(isset($val) && !empty($data) && is_array($data)) {
			$this->checkpk();
			$ret = Dxyz_DB::update($this->_table, $data, Dxyz_DB::field($this->_pk, $val), $unbuffered, $low_priority);
	...
	public function truncate() {
		Dxyz_DB::query("TRUNCATE ".Dxyz_DB::table($this->_table));
	}
	...
	public function fetch($id, $force_from_db = false){
		...
		if(!empty($id)) {
			if($force_from_db || ($data = $this->fetch_cache($id)) === false) {
				$data = Dxyz_DB::fetch_first('SELECT * FROM '.Dxyz_DB::table($this->_table).' WHERE '.Dxyz_DB::field($this->_pk, $id));
	...
	public function fetch_all_field(){
		$data = false;
		$query = Dxyz_DB::query('SHOW FIELDS FROM '.Dxyz_DB::table($this->_table), '', 'SILENT');
		if($query) {
			$data = array();
			while($value = Dxyz_DB::fetch($query)) {
	...
	public function range($start = 0, $limit = 0, $sort = '') {
		...
		return Dxyz_DB::fetch_all('SELECT * FROM '.Dxyz_DB::table($this->_table).($sort ? ' ORDER BY '.Dxyz_DB::order($this->_pk, $sort) : '').Dxyz_DB::limit($start, $limit), null, $this->_pk ? $this->_pk : '');
	}

	public function optimize() {
		Dxyz_DB::query('OPTIMIZE TABLE '.Dxyz_DB::table($this->_table), 'SILENT');
	}
